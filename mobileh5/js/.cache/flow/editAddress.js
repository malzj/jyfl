/*TMODJS:{"version":50,"md5":"33681661ed5ed6b87f3afcf2355f17e2"}*/
...
$out+='" placeholder="请输入"> </div> </div> <div class="mui-button-row margin_top_20"> <button type="button" class="mui-btn mui-btn-primary login_password_btn add_btn">保存</button> </div> </div> </div> <script> /* 启用选择器插件 */ jQuery.ajaxJsonp(web_url+\'/mobile/basic.php?act=getMobileCities\',{onlyCountry:1},function(result){ if(result.state==\'true\'){ var cityPicker = new mui.PopPicker({ layer: 2 }); var cityData = result.data; cityPicker.setData(cityData); var showCityPickerButton = document.getElementById(\'showCityPicker\'); var cityResult = document.getElementById(\'showCityPicker\'); var hcountry = document.getElementById(\'country\'); var hprovince = document.getElementById(\'province\'); showCityPickerButton.addEventListener(\'tap\', function(event) { cityPicker.show(function(items) { cityResult.value = items[0].text + " " + items[1].text; hcountry.value = items[0].value; hprovince.value = items[1].value; }); }, false); } }); /* 保存收货地址 */ jQuery(\'.mui-content\').on(\'tap\', \'.add_btn\', function(){ var consignee = jQuery(\'#consignee\').val(); var mobile = jQuery(\'#mobile\').val(); var country = jQuery(\'#country\').val(); var province = jQuery(\'#province\').val(); var address = jQuery(\'#address\').val(); var zipcode = jQuery(\'#zipcode\').val(); if( !consignee.trim() || !mobile.trim() || !country.trim()|| !province.trim()|| !province.trim() || !address.trim() || !zipcode.trim() ){ mui.alert(\'信息填写不完整\'); } jQuery.ajaxJsonp( web_url+\'/mobile/address.php\', { act:\'ajaxAddressSave\', consignee:consignee, mobile:mobile, country:country, province:province, address:address, zipcode:zipcode, address_id:';
$out+=$escape(data.consignee.address_id);
$out+=' }, function(data){ if(data.state == \'true\'){ mui.alert(\'保存成功！\',\'提示\',function(){ /* 保存成功跳转到下单页面 */ mui.back(); }); } }); }) </script>';
