...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    {
        /* 查询旧密码并与输入的旧密码比较是否相同 */
        $sql = "SELECT password FROM ".$ecs->table('users')." WHERE user_id = '$admin_id'";
        $old_password = $db->getOne($sql);
		$sql ="SELECT ec_salt FROM ".$ecs->table('users')." WHERE user_id = '$admin_id'";
    ...
    if($pwd_modified)
    {
        $sql = "UPDATE " .$ecs->table('users'). " SET ".
               "email = '$admin_email', ".
               "ec_salt = '$ec_salt' ". $password.
               "WHERE user_id = '$admin_id'";
		 $db->query($sql);
    }
   
        $sql = "UPDATE " .$ecs->table('supplier'). " SET ". $nav_list.
               "WHERE user_id = '$admin_id'";
   $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    exit;
}else if($_REQUEST['act'] == 'checkSuppLogin'){
	if ($_SESSION['supplier_id'])
	{
		echo json_encode(array('supplier_id'=>$_SESSION['supplier_id']));
	}
}
