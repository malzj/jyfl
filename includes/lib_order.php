...
function shipping_area_info($shipping_id, $region_id_list)
{
    $sql = 'SELECT s.shipping_code, s.shipping_name, ' .
            ...
            'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' .
                $GLOBALS['ecs']->table('shipping_area') . ' AS a, ' .
                $GLOBALS['ecs']->table('area_region') . ' AS r, ' .
				$GLOBALS['ecs']->table('region') . ' AS re ' .
...
/**
 * 取得某供货商配送方式对应于某收货地址的区域信息
 * @param   int     $supplier_id        供货商id
 * @param   int     $shipping_id        配送方式id
 * @param   array   $region_id_list     收货人地区id数组
 * @return  array   配送区域信息（config 对应着反序列化的 configure）
 */
function supplier_shipping_area_info($supplier_id, $shipping_id, $region_id_list)
{
    $sql = 'SELECT s.shipping_code, s.shipping_name, ' .
                's.shipping_desc, s.insure, s.support_cod, a.configure,a.shipping_times, r.region_id ' .
            'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' .
                $GLOBALS['ecs']->table('supplier_shipping_area') . ' AS a, ' .
                $GLOBALS['ecs']->table('supplier_area_region') . ' AS r, ' .
				$GLOBALS['ecs']->table('region') . ' AS re ' .
            "WHERE s.shipping_id = '$shipping_id' " .
            'AND r.region_id ' . db_create_in($region_id_list) .
            ' AND re.region_id = r.region_id AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1 AND a.supplier_id = '. $supplier_id . ' ORDER BY re.region_type DESC';
	$row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
        $shipping_config = unserialize_config($row['configure']);
        
		//var_dump($shipping_config );exit;
        if (isset($shipping_config['pay_fee']))
        {
            if (strpos($shipping_config['pay_fee'], '%') !== false)
            {
...
function shipping_fee($shipping_code, $shipping_config, $goods_weight, $goods_amount, $goods_number='')
{
    ...
    if (file_exists($filename))
    {
		...
		$shipping_fee = $obj->calculate($goods_weight, $goods_amount, $goods_number);

		if ($shipping_fee > 0){//如果运费不为0，查看分段设置的运费
			$arr_shippingFee = array();
			foreach ($shipping_config as $key=>$var){
				if ($var['name'] == 'order_shipping_fee'){
					if (!empty($var['value'])){
						foreach ($var['value'] as $k=>$v){
							if ($goods_amount >= $v['order_amount']){
								$arr_shippingFee[$v['order_amount']] = $v['shipping_fee'];
							}
						}
					}
				}
			}
			if (!empty($arr_shippingFee)){
				$shipping_fee = min($arr_shippingFee);
			}
		}
        return $shipping_fee;
    }
 ...
 * @return  float   运费
 */
function shipping_fee_open_time($suppller_id=0){
	if (isset($_SESSION['flow_shipping'][$suppller_id])){
		return $_SESSION['flow_shipping'][$suppller_id];
	}else{
		return 0;
	}
}
...
function order_goods($order_id)
{
    $sql = "SELECT og.rec_id, og.goods_id, og.goods_name, og.goods_sn, og.market_price, og.goods_number, " .
            ...
            "FROM " . $GLOBALS['ecs']->table('order_goods') .' AS og LEFT JOIN '.$GLOBALS['ecs']->table('goods').' AS g '.
            "ON og.goods_id = g.goods_id WHERE og.order_id = '$order_id'";
 ...
 * @return  array   订单商品数组
 */
function order_goods1($order_id)
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " .
            ...
            " WHERE order_id = '$order_id' and is_refund = 0";
...
function order_fee($order, $goods, $consignee)
{
    ...
    }elseif (!empty($order['supplier_bonus'])){
		foreach ($order['supplier_bonus'] as $key=>$var){
			if (!empty($var)){
				$bonus = bonus_info($var);
				$total['supplier_bonus_info'][$key]['money']    = $bonus['type_money'];
				$total['supplier_bonus_info'][$key]['bonus_id'] = $var;
				$total['supplier_bonus'] += $bonus['type_money'];
			}
			
		}
		$total['supplier_bonus_formated'] = price_format($total['supplier_bonus'], false);
	}
	...
	*/
	...
	if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0){
		 ...
		 * 需要选择配送时间的供应商。
		 * 思路：
		 * 	   购物车中的商品所属的供应商，如果不开启配送时间的设置，配送费用 shipping_fee()
		 * 	   如果供应商开启的配送时间，则使用 shipping_fee_open_time()
		 *
		 * @author  guoyunpeng
		 */
		/* $supplierInfo = array();
		...
		foreach ($goods as $val){
			...
			// 
			if (!empty($row_supp['supplier_id'])){
				$shipping_info = supplier_shipping_area_info($row_supp['supplier_id'], $order['shipping_id'], $region);
			// 没有供应商走这个， 这个永远都不会执行，因为所有商品都是经过供应商添加，总后台不添加、
			}else{
				$shipping_info = shipping_area_info($order['shipping_id'], $region);
			}
			 */
			...
			// 供应商配送信息为空，并且该供应商是开启配送时间的供应商
			if(in_array($row_supp['supplier_id'], $supplierInfo) && empty($shipping_info)){
				$total['supplier_shipping'][$row_supp['supplier_id']]['shipping_fee'] = shipping_fee_open_time($row_supp['supplier_id']);
				$total['supplier_shipping'][$row_supp['supplier_id']]['supplier_name'] = $row_supp['supplier_name2'];
				$total['supplier_shipping'][$row_supp['supplier_id']]['formated_shipping_fee'] = price_format($total['supplier_shipping'][$row_supp['supplier_id']]['shipping_fee'], true);
			} 
	...
	// 红包和积分最多能支付的金额为商品总额 + 运费
    $max_amount = $total['goods_price'] == 0 ? $total['goods_price'] : $total['goods_price'] - $bonus_amount + $total['shipping_fee'];
    ...
    else
    {
		...
		//减去供货商红包
		if (isset($total['supplier_bonus'])){
			$supplier_bonus = min($total['supplier_bonus'], $max_amount);
			$total['amount'] -= $supplier_bonus;// 还需要支付的订单金额
		}
...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, " .
            ...
            " as s ON s.supplier_id = g.supplier_id WHERE c.session_id = '" . SESS_ID . "' " .
    ...
    foreach ($arr as $key => $value)
    {
        ...
        $arr[$key]['goods_thumb'] = get_image_path($row['goods_id'], $goods_thumb, true);
    }
 ...
 *  @author		guoyunepng
 *  @access		public
 *  @param		string		$nember		规格id
 *  
 *  @return		string
 */
function get_goods_spec_info($nember,$price)
{
	$spec = '';	
	if (!empty($nember))
	{
...
function user_bonus($user_id, $goods_amount = 0)
{
    ...
    $sql = "SELECT t.type_id, t.type_name, t.bonus_name, t.type_money, t.supplier_id, b.bonus_id " .
            "FROM " . $GLOBALS['ecs']->table('bonus_type') . " AS t," .
                $GLOBALS['ecs']->table('user_bonus') . " AS b " .
            "WHERE t.type_id = b.bonus_type_id " .
            "AND t.use_start_date <= '$today' " .
			" AND CASE WHEN t.endday > 0 THEN (b.add_time + t.endday * 3600 * 24) >= '$today' ELSE t.use_end_date >= '$today' END ".
            //"AND t.use_end_date >= '$today' " .
            "AND t.min_goods_amount <= '$goods_amount' " .
            "AND b.user_id<>0 " .
            "AND b.user_id = '$user_id' " .
            "AND b.order_id = 0";
	$arr_bouns = $GLOBALS['db']->getAll($sql);
	$arr_supplierBonus = array();
	foreach ($arr_bouns as $key=>$bonus){
		$arr_supplierBonus[$bonus['supplier_id']]['supplier_id'] = $bonus['supplier_id'];
		$arr_supplierBonus[$bonus['supplier_id']]['supplier_name'] = empty($bonus['supplier_id']) ? '本网店' : $GLOBALS['db']->getOne('SELECT supplier_name FROM '.$GLOBALS['ecs']->table('supplier')." WHERE supplier_id = '".intval($bonus['supplier_id'])."'");
		$arr_supplierBonus[$bonus['supplier_id']]['bonus'][$bonus['bonus_id']]['type_id'] = $bonus['type_id'];
		$arr_supplierBonus[$bonus['supplier_id']]['bonus'][$bonus['bonus_id']]['type_name'] = $bonus['type_name'];
		$arr_supplierBonus[$bonus['supplier_id']]['bonus'][$bonus['bonus_id']]['bonus_name'] = $bonus['bonus_name'];
		$arr_supplierBonus[$bonus['supplier_id']]['bonus'][$bonus['bonus_id']]['type_money'] = $bonus['type_money'];
		$arr_supplierBonus[$bonus['supplier_id']]['bonus'][$bonus['bonus_id']]['bonus_id'] = $bonus['bonus_id'];
	}

	return array('all'=>$arr_bouns, 'supplier'=>$arr_supplierBonus);
    //return $GLOBALS['db']->getAll($sql);
}
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0)
{
    ...
    if (1 == $refund_type)
    {
        log_account_change($user_id, $amount, 0, 0, 0, $change_desc, ACT_OTHER, 1, $order);
    ...
    else
    {
        ...
        if ($user_id > 0)
        {
            ...
            $sql = "SELECT ua.*".
                    ...
                    " WHERE u.user_id='$user_id' AND ua.address_id = u.address_id AND country = '".$GLOBALS['int_cityId']."'";
...
function get_total_bonus()
{
    ...
    $today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
	  ...
	  $sql = "SELECT t.type_money,c.goods_number * c.goods_price as goods_total , t.type_id  " .
            "FROM " . $GLOBALS['ecs']->table('cart') . " AS c, "
	...
	//$arr_cardGoods = get_cart_goods();
	//$arr_supplierGoods = array();
	//foreach ($arr_supplierGoods['goods_list'] as $key=>$var){
	//	$arr_supplierGoods[$var['supplier_id']]['goods_amount'] += $var['subtota'];
	//}
    ...
    foreach ($res AS $row)
    {
		$bonus_list[$row['type_id']]['goods_amount'] += $row['goods_total'];
		$bonus_list[$row['type_id']]['type_money']   = $row['type_money'];
    }
	//var_dump($bonus_list);
	//exit;

	foreach ($bonus_list as $typeId => $goods){
		 $sql = "SELECT min_goods_amount_fa " .
            " FROM " . $GLOBALS['ecs']->table('bonus_type') .
            " WHERE type_id = '$typeId' LIMIT 1";
			$rows = $GLOBALS['db']->getRow($sql);
			//var_dump($rows['min_goods_amount_fa']);
			
			if($rows['min_goods_amount_fa'] == 0)
			{
				 ...
				 "FROM " . $GLOBALS['ecs']->table('cart') . " AS c, "
					...
					$goods_totala = $GLOBALS['db']->getOne($sql);

					

				$goods_total = $goods_total + $goods_totala;
				//var_dump($goods_totala);

			}else{
				if($goods['goods_amount'] >= $rows['min_goods_amount_fa']){
				
					
					$sqlb = "SELECT t.type_money as goods_bonus " .
				 "FROM " . $GLOBALS['ecs']->table('cart') . " AS c, "
                    . $GLOBALS['ecs']->table('bonus_type') . " AS t, "
                    . $GLOBALS['ecs']->table('goods') . " AS g " .
					"WHERE c.session_id = '" . SESS_ID . "' " .
					...
					$goods_totalb = $GLOBALS['db']->getOne($sqlb);

					$goods_total = $goods_total + $goods_totalb;

				//$goods_total = $goods_total + $row['type_money'];
				//var_dump($goods_totalb);
				}
			}
...
function send_order_bonus($order_id)
{
    ...
    if ($bonus_list)
    {
        ...
        foreach ($bonus_list AS $bonus)
        {
            ...
            /* 修改用户红包 */
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('user_bonus') . " (bonus_type_id, huoqu_orderid,user_id, add_time) " .
                    "VALUES('$bonus[type_id]', '$order_id', '$user[user_id]', '".gmtime()."')";
...
function order_bonus($order_id)
{
    ...
    $today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
	 ...
	 $sql = "SELECT b.type_id, b.type_money, o.goods_number, o.goods_number * o.goods_price as goods_total " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') . " AS o, " .
                      $GLOBALS['ecs']->table('goods') . " AS g, " .
    ...
    {
		$bonus_list[$row['type_id']]['goods_amount'] += $row['goods_total'];
		$bonus_list[$row['type_id']]['goods_number'] += $row['goods_number'];
		$bonus_list[$row['type_id']]['type_money']   = $row['type_money'];
    }
	...
	foreach ($bonus_list as $typeId => $goods){
		 $sql = "SELECT min_goods_amount_fa " .
			...
			if($rows['min_goods_amount_fa'] == 0)
			{
				 ...
				 $list[$int_index]['type_id'] = $typeId;
				 $list[$int_index]['type_money'] = $goods['type_money'];
				 $list[$int_index]['number'] = $goods['goods_number'];
				...
				if($goods['goods_amount'] >= $rows['min_goods_amount_fa']){
					...
					/*$sql = "SELECT b.type_id, b.type_money as type_money, SUM(o.goods_number) AS number " .
					 "FROM " . $GLOBALS['ecs']->table('order_goods') . " AS o, " .
                      $GLOBALS['ecs']->table('goods') . " AS g, " .
                      $GLOBALS['ecs']->table('bonus_type') . " AS b " .
					" WHERE o.order_id = '$order_id' " .
...
/*
	...
	*/
	...
	$arr_bonustype = $GLOBALS['db']->getAll($sql);
    $list = !empty($arr_bonustype) ? array_merge($list, $arr_bonustype) : $list;
...
function split_order($new_order_id){
	$sql = "select IF(g.supplier_id, g.supplier_id, 0) AS supplier_id, og.rec_id, og.goods_number, og.goods_price, g.goods_id, og.goods_attr_id from ". $GLOBALS['ecs']->table("order_goods") .
	...
	while ($row=$GLOBALS['db']->fetchRow($res))
	{
		...
		$split_orders[$row['supplier_id']]['bonus_id'] = $GLOBALS['total']['supplier_bonus_info'][$row['supplier_id']]['bonus_id'];

		$split_orders[$row['supplier_id']]['order_amount'] = $split_orders[$row['supplier_id']]['goods_amount'] + $split_orders[$row['supplier_id']]['shipping_fee'] - $split_orders[$row['supplier_id']]['bonus'];
		$split_orders[$row['supplier_id']]['order_amount_formated'] = price_format($split_orders[$row['supplier_id']]['order_amount']);
		
		// 销售比例
		if (!isset($split_orders[$row['supplier_id']]['shop_ratio']))
		{
		    $customSpec = null;
		    $specAttr = strpos($row['goods_attr_id'], ',') !== false ? explode(',', $row['goods_attr_id']) : array($row['goods_attr_id']);
		    ...
		    $ratios = get_spec_ratio_price( array('spec_nember'=>$customSpec, 'goods_id'=>$row['goods_id']) , true);
	...
	foreach ($split_orders AS $spkey => &$split){
		...
		// 指定供应商的配送区域
		$split['city'] = $best_time[$spkey]['city'];
...
function get_supplier_name($sid){
	$sql = "SELECT supplier_name FROM".$GLOBALS['ecs']->table('supplier').
 ...
 *  动网场地数据更新
 */
 function update_venues_state($user_id=0)
 {
     $where = '1';
     // 传用户id了就查询该用户的订单
     if(intval($user_id) > 0){
     ...
     $where .= ' AND state = 1';
     $sql = 'SELECT id,api_order_id FROM '.$GLOBALS['ecs']->table('venues_order').' WHERE '.$where;
     ...
     if (!empty($orders))
     {
         foreach($orders as $id){
            $orderIds[] = $id['api_order_id'];    
         }
         // 批量获取订单状态，一次最多获取20个
         $apiOrderIds = array_chunk($orderIds, 20);
         foreach ($apiOrderIds as $ids)
         {
             $result_orders = getDongSite('status', array('orderId'=> implode(',', $ids)));
             
             if ($result_orders['code'] == 0)
             {                 
                 foreach ($result_orders['body'] as $order)
                 {
                     $state = 0;
                     switch ($order['status'])
                     {
                         case 2:   $state=1;   break;   // 已支付
                         case 3:   $state=2;   break;   // 已退款
                         case 4:   $state=3;   break;   // 已完成
                     }                 
                    
                     // 更新订单状态
                     if ( in_array($state, array(2,3)))
                     {
                         $GLOBALS['db']->query('UPDATE '.$GLOBALS['ecs']->table('venues_order')." SET state=".$state." WHERE api_order_id = '".$order['orderId']."'");
                     }
                 }
             }
         }
...
/** 更新动网门票状态
 *  当  order_state = 1 or order_state = 2 or order_state = 0
 */
function update_piao_order_status($user_id=0){
	...
	$where .= ' AND order_state in(0,1,2)';

	$sql = 'SELECT order_id,order_state FROM '.$GLOBALS['ecs']->table('piao_order').' WHERE '.$where;
	$orders = $GLOBALS['db']->getAll($sql);
	...
	{
		foreach($orders as $id){
			 $result_orders = getDongapi('odetail', array('orderId'=>$id['order_id']));
			 if ($result_orders['status'] == 0) continue;
			 $temp_orders = $result_orders['orders']['order'];
			 
			 // 如果接口返回的订单状态和后台的订单状态不一样，更新数据库订单状态
			 if ($temp_orders['orderState'] != $id['order_state'])
			 {
			 	$GLOBALS['db']->query('UPDATE '.$GLOBALS['ecs']->table('piao_order')." SET order_state=".$temp_orders['orderState']." WHERE order_id = '".$id['order_id']."'");
			 }			
		}
	}
}
...
function update_piaoduoduo_order_status($user_id=0){
    ...
    $sql = 'SELECT order_sn,api_order_sn,Status FROM '.$GLOBALS['ecs']->table('piaoduoduo_order').' WHERE '.$where." order by add_time desc limit 0,30";
    $orders = $GLOBALS['db']->getAll($sql);
    ...
    {
        foreach($orders as $id){
             $result_orders = piaoduoduo('Agent_GetOrderInfoByKeyId', array('orderKeyId'=>$id['api_order_sn']));
            
             if ($result_orders['Status'] == 0) continue;
                       
             // 如果接口返回的订单状态和后台的订单状态不一样，更新数据库订单状态
            if ($result_orders['Status'] != $id['Status'])              
             {  
                $set=" Status=".$result_orders['Status'];
                if($result_orders['WithDrawAmount']){
                    $set.=",refund =".$result_orders['WithDrawAmount'];
                }
                $sql='UPDATE '.$GLOBALS['ecs']->table('piaoduoduo_order')." SET".$set."  WHERE order_sn = '".$id['order_sn']."'";
...
// 更新在线选座订单状态
function movie_orders_status($order_sn='', $user_id=0){
	...
	$where .= ' AND order_status in(1,2,3)';
	
	$sql = 'SELECT * FROM '.$GLOBALS['ecs']->table('seats_order').' WHERE '.$where;
	$orders = $GLOBALS['db']->getAll($sql);
	...
	if(count($orders) > 0){
		foreach($orders AS $order){
			// 处理15分钟后没有付款的订单，状态改成2
			if($order['order_status'] == 1){
				if($order['add_time']+15*60 < time()){
					$GLOBALS['db']->query('UPDATE '.$GLOBALS['ecs']->table('seats_order')." SET order_status=2 WHERE id = '".$order['id']."'");
				}
				continue;
			}
			// 订单状态是2（已取消），支付状态是0（未支付）
			if($order['order_status'] == 2 && $order['card_pay'] == 0){
				continue;
			}
			
			//error_log($order['order_sn']."\r\n", 3, 'error.log');
			$tmpOrders = getCDYapi(array('action'=>'order_Query', 'order_id'=>$order['order_sn']));
			if($order['order_status'] < $tmpOrders['orders'][0]['orderStatus']){
				$orderIds[$order['id']] = $tmpOrders['orders'][0]['orderStatus'];
			}
		}		
	}
	
	// 重复的值放在一起
	$uniqueArray = unique_array($orderIds);
	if(count($uniqueArray)){
		foreach($uniqueArray AS $unique){
			$orderIdsa = array_keys($unique);
			$orderStatus = array_values($unique);
			$orderStatus = current($orderStatus);
			$GLOBALS['db']->query('UPDATE '.$GLOBALS['ecs']->table('seats_order')." SET order_status = '".$orderStatus."' WHERE id IN(".implode(',',$orderIdsa).")");
		}
	}
		
}

function unique_array($arr=array(), &$return=array()){
	if(count($arr) == 0) return array();
	foreach($arr AS $key=>$val){
		if(is_array($val)){
			unique_array($val, $return);
			continue;
		}		
		$return[$val][$key] = $val;		
	}
