...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $goods_list = goods_list($_REQUEST['act'] == 'list' ? 0 : 1, -1);
...
elseif ($_REQUEST['act'] == 'Commodity_export')
{
    // $menus=goods_list(0,1);
    ...
    $sql = "SELECT g.*,s.supplier_name,b.brand_name,gs.spec_name,gs.spec_price,gs.spec_nember,c.cat_name FROM " . $GLOBALS['ecs']->table('goods') . " AS g ".
        ...
        " WHERE 1 $where" . " ORDER BY g.goods_id desc";
    ...
    // foreach ($res as $key => $rows) {
        # code...
        $list [$key] ['goods_id'] = $rows ['goods_id'];
        $list [$key] ['goods_name'] = $rows ['goods_name'];//商品名
        $list [$key] ['cat_name'] = $rows ['cat_name'];//分类
        $list [$key] ['brand_name'] = $rows ['brand_name'];//品牌
        $list [$key] ['spec_name'] = $rows ['spec_name'];//规格名
        $list [$key] ['spec_price'] = $rows ['spec_price'];//价格
        $list [$key] ['spec_nember'] = $rows ['spec_nember'];//规格编号
        $list [$key] ['is_on_sale'] = $rows ['is_on_sale']?'上架':'下架';
        $list [$key] ['goods_number'] = $rows ['goods_number'];//库存
        $list [$key] ['supplier_name'] = $rows ['supplier_name'];//供应商
        $list [$key] ['add_time'] = local_date('Y-m-d H:i:s',$rows ['add_time']);//上架时间
        $list [$key] ['last_update'] = local_date('Y-m-d H:i:s',$rows ['last_update']);//更新时间
        $key++;
...
// die;   
    ...
    foreach ( $list as $key => $val ) {
        ...
        $exportInfo [] = array (
                ...
                'C' . $row =>  $val ['cat_name'],
                'D' . $row =>  $val ['brand_name'],
                'E' . $row => ' ' . $val ['spec_name'],
                'F' . $row =>  $val ['spec_price'],
                'G' . $row => ' ' . $val ['spec_nember'],
                'H' . $row =>  $val ['is_on_sale'],
                'I' . $row => ' ' .$val ['goods_number'],
                'J' . $row =>  $val ['supplier_name'],                
                'K' . $row =>  $val ['add_time'],
                'L' . $row =>  $val ['last_update']
        );
...
// die;    
    // 导入excel类
    include_once(ROOT_PATH. 'admin/includes/lib_autoExcels.php');
    $autoExcels = new autoExcels ( 'Excel2007' );
    $fileNmae = 'goods-' . date ( 'mdHis', time () ) . '.xlsx';
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
	...
	$action_link_supplier = $is_add ? array('href' => 'goods.php?act=list&supplier_status=0' , 'text' => '添加'): array('href' => 'goods.php?act=list&supplier_status='.$_REQUEST['supplier_status'] , 'text' => '添加');
	...
	// 商店折扣比例
	/* $shop_ratio = suppliers_ratio('shop_ratio');
	$smarty->assign('shop_ratio', $shop_ratio); */
    ...
    if ($is_add)
    {
		...
		// TODO 添加的话，默认默认显示一个
        $smarty->assign('spec_list_region',array(array()));
    ...
    else
    {
        ...
        $region_list = get_region();
        // 规格列表
        $specSql = "SELECT * FROM " . $ecs->table('goods_spec') . " WHERE goods_id = '$_REQUEST[goods_id]' ORDER BY id ASC";
        $arr_region = array();
        $goodsSpec = $db->getAll($specSql);
        if (!empty($goodsSpec))
        {   
        	foreach($goodsSpec as $gspec)
        	{
        		$gspec['region'] = $region_list;
        		// 规格对应的上线城市
        		$spec_region = $db->getAll(" SELECT region_id FROM ".$ecs->table('goods_spec_region'). ' WHERE spec_id = '.$gspec['id']);	
        		$new_region_ids = array();
        		foreach($spec_region as $region){
        			$new_region_ids[] = $region['region_id']; 
        		}
        		foreach($gspec['region'] as &$rlist){
        			$rlist['active'] = 0;
        			if (in_array($rlist['region_id'], $new_region_ids))
        			{
        				 $rlist['active'] = 1;
        			}        			
        		}
        ...
        if (empty($arr_region))
        {
        	$arr_region = array(array('region'=>$region_list));
        }
        $smarty->assign('spec_list_region',$arr_region);       
    ...
    // TODO 供应商分类显示，只有实体店的商户才显示值属于他的分类。
    if ($sinfo['is_entity'] == 1)
    {
		$smarty->assign('supplier_category_list', category_list($goods['cat_id']));    	
    }
	...
	//货号区域
	$arr_province = get_regions();
	$smarty->assign('arr_province',       $arr_province);
	$arr_goods_huohao = '';
	if(isset($_REQUEST['goods_id'])){
		$arr_goods_huohao = $db->getAll('SELECT * FROM '.$ecs->table('goods_huohao')." WHERE goods_id = '".intval($_REQUEST['goods_id'])."' ORDER BY huohao_id ASC");
	}
	if (empty($arr_goods_huohao)){
		$arr_goods_huohao = array('0' => array('goods_sn'=>'', 'province'=>'', 'city'=>'', 'district'=>''));
	}else{
		foreach ($arr_goods_huohao as $key=>$var){
			$arr_regions = explode(',', $var['region_ids']);
			foreach ($arr_province as $k=>$v){
				if (in_array($v['region_id'], $arr_regions)){
					$var['check'][$v['region_id']] = ' checked="checked"';
				}
			}
			$arr_goods_huohao[$key] = $var;
		}
	}
	$smarty->assign('arr_goods_huohao', $arr_goods_huohao);
...
{
    $code = empty($_REQUEST['extension_code']) ? '' : trim($_REQUEST['extension_code']);
	$is_copy = intval($_REQUEST['is_copy']);//是否为复制
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "seller_note, goods_weight, goods_number, goods_num, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number', '$goods_num',".
        ...
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, goods_num, warn_number, integral, give_integral, is_best, is_new, is_hot, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, supplier_id, supplier_status)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number', '$goods_num',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$supplier_id','0')";
        }
    }
    else
    {
	...
	if ($is_copy){
		//获取原来商品图片信息
		$arr_goods = $db->getRow('SELECT goods_img, original_img, goods_thumb FROM '.$ecs->table('goods')." WHERE goods_id='".intval($_REQUEST['goods_id'])."'");
		if (empty($goods_img)){
			//如果没有上传商品调用原来商品图片
			$db->query('UPDATE '.$ecs->table('goods')." SET goods_img = '".$arr_goods['goods_img']."', original_img='".$arr_goods['original_img']."' WHERE goods_id = '".intval($goods_id)."'");
		}
		if (empty($goods_thumb)){
			//没有商品缩略图使用原来商品缩略图
			$db->query('UPDATE '.$ecs->table('goods')." SET goods_thumb = '".$arr_goods['goods_thumb']."' WHERE goods_id = '".intval($goods_id)."'");
		}
		if (isset($_POST['old_img_desc'])){
			foreach ($_POST['old_img_desc'] AS $img_id => $img_desc){
				$sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
					" SELECT '$goods_id', img_url, img_desc, thumb_url, img_original FROM ".$GLOBALS['ecs']->table('goods_gallery')." WHERE img_id = '$img_id'";
	...
	if (empty($_POST['goods_sn'])){
		...
		if (!$is_insert){
			//如果是编辑则删除原来商品货号
			$db->query('DELETE FROM '.$ecs->table('goods_huohao')." WHERE goods_id = '$goods_id'");
		}
		$db->query('UPDATE '.$ecs->table('goods')." SET goods_sn = '' WHERE goods_id = '$goods_id'");
		//插入商品货号
		foreach ($_POST['goods_sn'] as $key => $var){
			if (!empty($var)){
				$str_goodsSn = $var;
			}else{
				$max_id      = $is_insert ? $db->getOne("SELECT MAX(goods_id) + 1 FROM ".$ecs->table('goods')) : $_REQUEST['goods_id'];
				$str_goodsSn = generate_goods_sn($max_id);
			}
			$arr_regions = $_POST['region'][$key];
			$str_regions = !empty($arr_regions) ? implode(',', $arr_regions) : '';
			$db->query('INSERT INTO '.$ecs->table('goods_huohao')." (goods_id, goods_sn, region_ids) VALUES ('$goods_id', '$str_goodsSn', '$str_regions')");
		}
	}
    ...
    {
    	// 如果没有选择默认显示的规格，就指定第一个规格
    	if (check_default_show($_POST['default_show']) === false)
    	{
    		$_POST['default_show'][0] = 1;
    	}
    	// 删除去掉的规格
    	$goods_specs = $GLOBALS['db']->getAll('SELECT * FROM '.$GLOBALS['ecs']->table('goods_spec').' WHERE goods_id ='.$goods_id);
    	$delete_specs = array();
    	foreach( $goods_specs as $row9 )
    	{ 
    		if (!in_array($row9['id'], $_POST['specs_id']))
    		{
    			$delete_specs[] = $row9['id'];
    		}
    	}
    	if (!empty($delete_specs))
    	{
    		$GLOBALS['db']->query("DELETE FROM ".$GLOBALS['ecs']->table('goods_spec')." WHERE id IN(".implode(',', $delete_specs).")");
    	}
    	
    	foreach($_POST['spec_name'] as $sk=>$sv){
    		// 规格是空，或价格是空，不做任何处理
    		$sv = trim($sv);
    		$sp = trim($_POST['spec_price'][$sk]);
    		if (empty($sv) || empty($sp))
    		{
    			continue;
    		}
    		...
    		// 规格参数+值的处理
    		$spec_name 		= $sv;    		
    		$default_show 	= intval($_POST['default_show'][$sk]);
    		$spec_price 	= $_POST['spec_price'][$sk];
    		
    		if (empty($_POST['spec_no'][$sk]))
    		{
    			$spec_nember = $goods_id.mt_rand(100, 999);
    		}
    		else {
    			$spec_nember = $_POST['spec_no'][$sk];
    		}
    		
    		$specs_id = intval($_POST['specs_id'][$sk]); 
    		
    		// 上线城市
    		$spec_city = empty($_POST['spec_city'][$sk]) ? array() : $_POST['spec_city'][$sk];
    		$region_lists = array_merge($spec_city, $region_lists);
    		
    		
    		if ($specs_id > 0)
    		{
    			// 更改规格信息
    			$update_sql = "UPDATE ".$ecs->table('goods_spec')." SET ".
			    				"spec_name = '$spec_name', ".
			    				"spec_price = '$spec_price', ".
			    				"spec_nember = '$spec_nember', ".    				
			    				"default_show = '$default_show' ".
    							"WHERE id = ".$specs_id;
    			$db->query($update_sql);
    				
    			// 更改城市
    			$region_data = $db->getAll("SELECT region_id FROM ".$ecs->table('goods_spec_region')." WHERE spec_id =".$specs_id);
    			if (!empty($region_data))
    			{
    				$region_ids = array();
    				foreach((array)$region_data as $rdata){
    					$region_ids[] = $rdata['region_id'];
    				}
    				// 增加的城市
    				$idiff = array_diff($spec_city, $region_ids);    				
    				$insertValue = array();
    				foreach($idiff as $scity){	$insertValue[] = "( NULL,'".$specs_id."', '".$scity."')";}
    				if (!empty($insertValue))
    				{  					
    					$region_sql = "INSERT INTO ".$ecs->table('goods_spec_region')." VALUES".implode(',',$insertValue);
    					$db->query($region_sql);
    				}
    				// 去掉的城市
    				$ddiff = array_diff($region_ids, $spec_city);
    				$deleteValue = array();
    				foreach($ddiff as $scity){	$deleteValue[] = $scity; }
    				if (!empty($deleteValue))
    				{    						
    					$db->query("DELETE FROM ".$ecs->table('goods_spec_region')." WHERE region_id IN (".implode(',', $deleteValue).") AND spec_id =".$specs_id);
    				}    					
    			}
    			else {
    				$insertValue = array();
    				foreach($spec_city as $scity){	$insertValue[] = "( NULL,'".$specs_id."', '".$scity."')";}
    				if (!empty($insertValue))
    				{
    					$region_sql = "INSERT INTO ".$ecs->table('goods_spec_region')." VALUES".implode(',',$insertValue);
    					$db->query($region_sql);
    		...
    		{
    			$spec_insert_sql = "INSERT INTO ".$ecs->table('goods_spec').
    			" (spec_name, spec_price, spec_nember, goods_id, default_show, supplier_id)".
    			" VALUES('".$spec_name."','".$spec_price."','".$spec_nember."','".$goods_id."','".$default_show."', '".$supplier_id."')";
    			$db->query($spec_insert_sql);
    				 
    			$insert_spec_id = $db->insert_id();
    			// 没有上线城市退出单次循环
    			if (empty($spec_city)) continue;
    				 
    			$scityValue = array();
    			// 上线城市处理
    			foreach($spec_city as $scity){
    				$scityValue[] = "( NULL,'".$insert_spec_id."', '".$scity."')";
    			}
    			// 添加上线城市
    			$region_sql = "INSERT INTO ".$ecs->table('goods_spec_region')." VALUES".implode(',',$scityValue);
    			$db->query($region_sql);
    		} 
    	}
    }
    
    // 更新上线城市到商品表里的 region_ids
	if (!empty($region_lists))
	{
		$new_rigion_lists = implode(',',array_unique($region_lists));
		$db->query("UPDATE ".$ecs->table('goods')." SET region_ids = '".$new_rigion_lists."' WHERE goods_id = ".$goods_id);
	}   
...
// 用户的分类信息
function category_list( $cat_id)
{
	$pid = 0;
	
	$list = $GLOBALS['db']->getAll(" SELECT * FROM ".$GLOBALS['ecs']->table('category'));
	$supplier_info = supplier_info();
	if (!empty($list))
	{
		foreach($list as $lis){
			if ($lis['cat_name'] == $supplier_info['supplier_name'])
			{
				$pid = $lis['cat_id'];
			}
		}
	...
	// 查找它的子类
	$cat_list = array();
	foreach ($list as $ink=>$ins)
	{
		if ($ins['parent_id'] == $pid)
		{
			$cat_list[$ink]['cat_id'] = $ins['cat_id'];
			$cat_list[$ink]['cat_name'] = $ins['cat_name'];
		}
	}
	
	// select 格式
	$html = '';
	foreach($cat_list as $clt)
	{
		if ($clt['cat_id'] == $cat_id)
		{
			$html .= '<option value="'.$clt['cat_id'].'" selected>'.$clt['cat_name'].'</option>';
		}
		else {
			$html .= '<option value="'.$clt['cat_id'].'" >'.$clt['cat_name'].'</option>';
		}
...
function check_default_show($show)
{
	$return = false;
	foreach($show as $val)
	{
