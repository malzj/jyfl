...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
            ...
            'ORDER BY a.sort_order, ceiling(g.attr_price),  g.goods_attr_id';
...
function get_card_rule_ratio($catid=0){
	...
	// 卡在规则里，但当前分类不在卡规则里，显示错误信息，返回首页。
	// 卡在规则里，当前分类也在卡规则里，但当前城市不在不支持，显示错误信息，返回首页。
	// 在卡规则里
	if ( !empty($navinfo) )
	{
		$region = $nav_ids = array();
		foreach( (array)$navinfo as $nav){
			$nav_ids[] = $nav['nav_id'];
			if (strpos($nav['region'], ',') !== false)
			{
				$ids = explode(',', $nav['region']);
				$newRegion = array_merge( (array)$region[$nav['nav_id']], $ids );
				$region[$nav['nav_id']] = $newRegion;
			}
			else
			{
				$newRegion = array_merge( (array)$region[$nav['nav_id']], array($nav['region']) );
				$region[$nav['nav_id']] = $newRegion;
			}
		}
				
		$navList = $GLOBALS['db']->getAll('SELECT id,cid FROM '.$GLOBALS['ecs']->table('nav').' WHERE id IN('.implode(',', $nav_ids).')');
		foreach( (array)$navList as $nl)
		{
			if($nl['cid'] > 0)
			{
				$cat_ids[$nl['id']]   = $nl['cid'];
			}
			else {
				$not_cat_ids[$nl['id']] = $nl['id'];
			}
		}
		
		// 如果是接口，判断有没有权限
		if ( interface_navid($catid) !== false)
		{
			$interface = interface_navid($catid);
			if ( !in_array($interface, $not_cat_ids))
			{
				show_message('你没有权限访问该栏目！(error:1)');
			}
		}
		
		// 当前分类id对应的导航id。
		$new_cat_ids = array_flip($cat_ids);
		
		if ( (!in_array($catid, $cat_ids) && !isset($interface) ) ||
			 (in_array($catid, $cat_ids) && !in_array($int_cityId, $region[$new_cat_ids[$catid]]) && !isset($interface) ))
		{
			show_message('你没有权限访问该栏目！(error:2)');
		}
	...
	{
		//$nsql = 'SELECT * FROM '. $GLOBALS['ecs']->table('nav') . ' WHERE ifshow = \'1\' AND type= \'middle\' ORDER BY type, vieworder';
		$nsql = 'SELECT * FROM '. $GLOBALS['ecs']->table('nav') . ' WHERE type= \'middle\' ORDER BY type, vieworder';
		...
		if ( interface_navid($catid) !== false)
		{
			...
			if ( !in_array($interface, $not_cat_ids) )
			{				
				show_message('你没有权限访问该栏目！(error:3)');
			}			
		}
		else
		{
			...
			{				
				show_message('你没有权限访问该栏目！(error:4)');
			}
		}	 
	}
	...
	$new_cat_ids 		= array_flip($cat_ids);
	$new_not_cat_ids 	= array_flip($not_cat_ids); 
	
	if ($interface == false)
	{
		if (isset($new_cat_ids[$catid]))
		{
			$new_catid = $new_cat_ids[$catid];
		}
	}
	else {		
		$new_catid = $interface;
 ...
 *  @param  $f  保留小数点
 */
function interface_price($p,$t,$f=true){
	...
	if ( $f === false )
		...
		return number_format(round($price,1),1);
}

/**
 *  接口id对应的导航id
 */
function interface_navid($id=0){
...
/**
 *  处理并返回指定规格最终的售价
 *  @param	$spec	       array	规格信息	spec_nember 规格号，goods_id 商品id
 *  @param  $returnRatio   int      返回数据源， false，返回规格最终价格，true，返回三个比例
 */
function get_spec_ratio_price($spec, $returnRatio=false)
{
	// 当前登录账户的卡规则id
	$card_id = isset($_SESSION['card_id']) ? $_SESSION['card_id'] : 0 ;
	
	// 规格号和商品id不能我空
	if (empty($spec['goods_id']) || empty($spec['spec_nember']))
	{
		return false;
	}
	...
	$spec_info = $GLOBALS['db']->getRow("SELECT gs.*,g.cat_id,g.supplier_id FROM ".$GLOBALS['ecs']->table('goods_spec')." AS gs ".
				"LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ON g.goods_id = gs.goods_id ".
			 	"WHERE gs.goods_id=".$spec['goods_id']." AND gs.spec_nember ='".$spec['spec_nember']."'");
	...
	{	   
	   ...
	   // 单品销售比例
	   $card_spec = $GLOBALS['db']->getRow("SELECT * FROM ".$GLOBALS['ecs']->table('card_discount')." WHERE card_id=".$card_id." AND spec_id = ".$spec_info['id']);
	   if (!empty($card_spec))
	   {
	       $ratios['unit_ratio'] = $card_spec['discount'];
	   }
	}
 ...
 */
function get_show_specs($goods_id)
{
	$specs = get_specs($goods_id);
	$specId = array();
	if (!empty($specs))
	{
		...
		{
			$region_ids = array();
			$row = $GLOBALS['db']->getAll("SELECT region_id FROM ".$GLOBALS['ecs']->table('goods_spec_region')." WHERE spec_id=".$s_v['id']);
			foreach ($row as $rid)
			{
				$region_ids[] = $rid['region_id'];
			}
			if (!in_array($_SESSION['cityid'], $region_ids))
			{
				unset($specs[$s_k]);
			}
			$specId[] = $s_k; 
	...
	if (count($specs) > 1)
	{
		array_multisort($specId,SORT_ASC,$specs);
	}
	else if(!empty($specs))
	{
		reset($specs);
		$new_specs = current($specs);
		unset($specs);
		$specs[] = $new_specs;
	}
...
function get_specs($goods_id)
{
	...
	// 规格数据
	$specs = $GLOBALS['db']->getAll("SELECT * FROM ".$GLOBALS['ecs']->table('goods_spec').' WHERE goods_id='.$goods_id);
	...
	// 过滤掉卡规则屏蔽的规则
	$spec_out = $GLOBALS['db']->getAll("SELECT spec_id FROM ".$GLOBALS['ecs']->table('card_out')." WHERE card_id =".$card_id." AND goods_id=".$goods_id);
	...
	// 卡规则排除的规格id
	$not_spec_id = array();
	foreach ($spec_out as $out)
	{
		$not_spec_id[] = $out['spec_id']; 
	}
	
	// 筛选出可以显示的规格 
	if (!empty($not_spec_id))
	{
		foreach ($specs as $s_k=>$s_v)
		{
			if (in_array($s_v['id'], $not_spec_id))
			{
				unset($specs[$s_k]);
			}
		}	
