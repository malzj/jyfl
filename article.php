...
{
// echo "asdfas";
 //ecs_header("location:article_index.php?id=$article_id\n");
 header("Location: article_index.php?id=$article_id\n");
 exit;
}
...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
	...
	if($article['cat_id'] == 2 ){
	$smarty->assign('article_categories2',    get_cat_articles(2));
	}
	else{
	 $smarty->assign('article_categories2',   get_cat_articles(3)); //文章分类树
	}
...
function article_tree($cat_id = 0)
{
    ...
    */
    ...
    foreach ($res AS $row)
    {
        ...
        $cat_arr[$row['cat_id']]['name'] = $row['cat_name'];
		 if ($row['title'] != NULL)
        {
            $cat_arr[$row['cat_id']]['children'][$row['article_id']]['title']   = $row['title'];
            $cat_arr[$row['cat_id']]['children'][$row['article_id']]['article_id'] = $row['article_id'];
