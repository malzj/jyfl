...
function Focus() {
	...
	var timer = null;
	var oFocus = byid('tFocus01');
	var oPic = byid('tFocus-pic01');
	var oPicLis = bytag('li', oPic);
	var oBtn = byid('tFocus-btn01');
	...
	byid('tFocus-leftbtn01').onclick = byid('prev01').onclick = function() {
		iActive--;
		if (iActive == -1) {
	...
	byid('tFocus-rightbtn01').onclick = byid('next01').onclick = function() {
		iActive++;
	...
	function changePic() {
		...
		oBtnLis[iActive].className = 'active01';
		if (iActive == 0) {
			doMove(bytag('ul', oBtn)[0], 'left', 0);
		} else if (iActive >= oPicLis.length ) {
			doMove(bytag('ul', oBtn)[0], 'left', -(oPicLis.length - 3) * (oBtnLis[0].offsetWidth + 32));
		} else {
			doMove(bytag('ul', oBtn)[0], 'left', -(iActive - 1) * (oBtnLis[0].offsetWidth + 32));
		}
	};
	function autoplay() {
	...
	byid('tFocus01').onmouseover = function() {
		//clearInterval(aTimer);
	}
	byid('tFocus01').onmouseout = function() {
		//aTimer = setInterval(autoplay, 2000);
	}
}