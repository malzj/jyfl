	...
	if($content){
		//if($istitle or $iscomment){
			...
			if($data['data']['itemInfoModel']['picsPath']){
				...
				foreach($data['data']['itemInfoModel']['picsPath'] as $k=>$img){
					if($k>=$num) break;
					$imgArr[$k] = str_replace("../","",getImg($img));
				}
			}
			if($iscomment){
		...
		if($_GET['do'] == 1){
			...
			$db->query("update {$ecs->table('goods')} set 
				goods_desc='{$content}',goods_thumb='{$imgArr[0]}',goods_img='{$imgArr[0]}'{$uptitle} 
				where goods_id={$gid}");
			foreach($imgArr as $v){
				$db->query("insert into {$ecs->table('goods_gallery')} (goods_id,img_url,thumb_url) value ($gid,'{$v}','{$v}')");
			}
