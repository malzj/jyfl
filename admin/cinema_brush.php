...
{	
	...
	error_log(var_export($list, true),'3','error.log'); exit;
	$smarty->assign('cinema_list',  $list['cinema']);
    $smarty->assign('filter',       $list['filter']);
    $smarty->assign('record_count', $list['record_count']);
    $smarty->assign('page_count',   $list['page_count']);
...
elseif ($_REQUEST['act'] == 'update')
{
	...
	else
	{
		...
		$GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('cinema_list')." SET ".$set." WHERE komovie_cinema_id = '$searchCinemaid'");
	}
...
elseif ($_REQUEST['act'] == 'searchCinema')
{
	...
	foreach ($cinema as $value)
	{
		...
		$html .='<option value="'.$value['komovie_cinema_id'].'">'.$value['cinema_name'].'----【'.$value['cinema_address'].'】----'.$seat.'</option>';
	}
