...
<!-- 淇敼 by www.68ecshop.com 鐧惧害缂栬緫鍣�begin -->
...
<!-- 淇敼 by www.68ecshop.com 鐧惧害缂栬緫鍣�end -->
...
<div class="tab-div">
    ...
    <div id="tabbar-div">
      <p>
        ...
        <?php if ($this->_var['sinfo']['is_entity'] == 2): ?>
        
        <span class="tab-back" id="huohao-tab">商品货号</span>
    ...
    <div id="tabbody-div">
      <form enctype="multipart/form-data" action="" method="post" name="theForm" >
		...
		<input type="hidden" name="is_copy" value="<?php echo $this->_var['is_copy']; ?>" />
        <!-- 闁氨鏁ゆ穱鈩冧紖 -->
          ...
          <tr>
            ...
            <td>
            	<select name="cat_id" onchange="hideCatDiv()" >
            		...
            		<?php if ($this->_var['sinfo']['is_entity'] == 1): ?> <?php echo $this->_var['supplier_category_list']; ?> <?php else: ?> <?php echo $this->_var['cat_list']; ?> <?php endif; ?>
            	</select>
        ...
        <?php echo $this->html_options(array('options'=>$this->_var['suppliers_list_name'],'selected'=>$this->_var['goods']['supplier_id'])); ?>
      </select></td>
          </tr>
         <?php endif; ?>
         
         <!-- 实体店价格手动填写，不计算商城售价 -->
         <?php if ($this->_var['sinfo']['is_entity'] == 1): ?>
         <tr>
            <td class="label">实体店价：</td>
            <td><input type="text" name="market_price" value="<?php echo $this->_var['goods']['market_price']; ?>" size="20" />
              <!-- <input type="button" value="计算商城售价" onclick="marketPriceSetted()" /> -->
              <?php echo $this->_var['lang']['require_field']; ?>
            </td>
          </tr>
          
          <tr>
            <td class="label">商城扣点价：</td>
...
?>
		...
		<!-- 鍟嗗搧璐у彿 -->
		<!-- <table width="90%" id="huohao-table" style="display:none">
			<tr id="sn-table">
				<td align="left">
					<table width="100%" id="tbody-huohao" align="center">
						<?php $_from = $this->_var['arr_goods_huohao']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'huohao');$this->_foreach['huohao'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['huohao']['total'] > 0):
    foreach ($_from AS $this->_var['huohao']):
        $this->_foreach['huohao']['iteration']++;
?>
						<tr>
							<td valign="top" align="left">
								<?php if ($this->_foreach['huohao']['iteration'] == 1): ?>
								<a href="javascript:;" onclick="addGoodsSn(this)" style="float:left;">[+]</a>
								<?php else: ?>
								<a href="javascript:;" onclick="removeGoodsSn(this)" style="float:left;">[−]</a>
								<?php endif; ?>
								<table style="float:left;" width="90%" border="0" cellpadding="0" cellspacing="0">
									<tr>
										...
										<td valign="top" width="160">
											<input type="text" name="goods_sn[<?php echo ($this->_foreach['huohao']['iteration'] - 1); ?>]" value="<?php echo htmlspecialchars($this->_var['huohao']['goods_sn']); ?>" size="20" />
										</td>
										<td valign="top" width="50">
											<span style="font-weight:bold;">&nbsp;&nbsp;区域</span>
										</td>
										<td valign="top">
											<label><input type="checkbox" name="checkall" value="" onclick="checkAreaAll(this,0)" /> 全部</label>
											<?php $_from = $this->_var['arr_province']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'province');$this->_foreach['province'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['province']['total'] > 0):
    foreach ($_from AS $this->_var['province']):
        $this->_foreach['province']['iteration']++;
?>
											<label><input type="checkbox" name="region[<?php echo ($this->_foreach['huohao']['iteration'] - 1); ?>][]" value="<?php echo $this->_var['province']['region_id']; ?>"<?php echo $this->_var['huohao']['check'][$this->_var['province']['region_id']]; ?> /><?php echo $this->_var['province']['region_name']; ?></label>
											<?php if ($this->_foreach['province']['iteration'] % 15 == 0): ?><br/><?php endif; ?>
											<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
										</td>
									</tr>
								</table>
		...
		<table width="900" id="huohao-table" style="display:none; margin:0 auto">
			<tr>
				<td>
					<table id="tbody-huohao">
					<?php $_from = $this->_var['spec_list_region']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'spec_list');$this->_foreach['spec_list'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['spec_list']['total'] > 0):
    foreach ($_from AS $this->_var['spec_list']):
        $this->_foreach['spec_list']['iteration']++;
?>	
						<tr>
							<td align="right">															
								<?php if ($this->_foreach['spec_list']['iteration'] == 1): ?>
								<a href="javascript:;" onclick="addGoodsSn(this)" style="float:left; line-height:30px;">[+]</a>
								<?php else: ?>
								<a href="javascript:;" onclick="removeGoodsSn(this)" style="float:left; line-height:30px;">[−]</a>
								<?php endif; ?>
																
								<table width="750" style="margin:0 auto;">
									<tr>
										<td>规格名：<input type="text" name="spec_name[<?php echo ($this->_foreach['spec_list']['iteration'] - 1); ?>]" value="<?php echo $this->_var['spec_list']['spec_name']; ?>" size="20"></td>
										<td >规格价格：<input type="text" name="spec_price[<?php echo ($this->_foreach['spec_list']['iteration'] - 1); ?>]" value="<?php echo $this->_var['spec_list']['spec_price']; ?>" size="8"></td>
										<td >规格编号：<input type="text" name="spec_no[<?php echo ($this->_foreach['spec_list']['iteration'] - 1); ?>]" value="<?php echo $this->_var['spec_list']['spec_nember']; ?>" size="10"></td>
										<td >默认显示：<input type="checkbox" name="default_show[<?php echo ($this->_foreach['spec_list']['iteration'] - 1); ?>]" value="1" <?php if ($this->_var['spec_list']['default_show'] == 1): ?> checked <?php endif; ?>></td>
										<td ><input type="hidden" name="specs_id[<?php echo ($this->_foreach['spec_list']['iteration'] - 1); ?>]" value="<?php echo $this->_var['spec_list']['id']; ?>"><a href="javascript:;" onclick="shwoRegion(this)">上线城市</a></td>
									</tr>									
									<tr style="display:none;">
										<td colspan="5">
										  <span style="display:inline-block; width:80px"><input type="checkbox" onclick='listTable.selectAll(this, "spec_city[<?php echo ($this->_foreach['spec_list']['iteration'] - 1); ?>][]")'>全选 </span>
...
?>
										  <span style="display:inline-block;width:80px;"><input type="checkbox" name="spec_city[<?php echo ($this->_foreach['spec_list']['iteration'] - 1); ?>][]" value="<?php echo $this->_var['region']['region_id']; ?>" /><?php echo $this->_var['region']['region_name']; ?></span>
										  <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
										  <?php else: ?>
										  <?php $_from = $this->_var['spec_list']['region']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'region');if (count($_from)):
    foreach ($_from AS $this->_var['region']):
?>
										  	 <span style="display:inline-block;width:80px;"><input type="checkbox" name="spec_city[<?php echo ($this->_foreach['spec_list']['iteration'] - 1); ?>][]" value="<?php echo $this->_var['region']['region_id']; ?>" <?php if ($this->_var['region']['active'] == 1): ?> checked <?php endif; ?> /><?php echo $this->_var['region']['region_name']; ?></span>
										  <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
										  <?php endif; ?>								 
										</td>																			
									</tr>
        ...
        <table width="90%" id="mix-table" style="display:none" align="center">
		  ...
		  <tr>
            ...
            <td><input type="text" name="goods_num" value="<?php echo $this->_var['goods']['goods_num']; ?>" size="20" /></td>
          </tr>
...
<!--浠ｇ爜淇敼_start By www.ecshop68.com  灏�鍟嗗搧鐩稿唽 杩欓儴鍒嗕唬鐮佸畬鍏ㄤ慨鏀规垚涓嬮潰杩欐牱-->
...
?>
              <div id="gallery_<?php echo $this->_var['img']['img_id']; ?>" style="float:left; text-align:center; border: 1px solid #DADADA; margin: 4px; padding:2px;">
                <a href="javascript:;" onclick="dorpImgConf(<?php echo $this->_var['img']['img_id']; ?>);">[-]</a><br />
			  ...
			  <!--
				function dorpImgConf(imgid){
					var copyLang = '<?php echo $this->_var['is_copy']; ?>' ? '鍒犻櫎璇ュ浘鐗囧師鍟嗗搧璇ュ浘鐗囦篃灏嗗垹闄わ紝' : '';
					if (confirm(copyLang + '<?php echo $this->_var['lang']['drop_img_confirm']; ?>')){
						 dropImg(imgid);
					}else{
						return false;
					}
		...
		<!--浠ｇ爜淇敼_end By www.ecshop68.com-->
...
?>
        ...
        <div class="button-div">
		  ...
		  <!--淇敼浠ｇ爜_start By www.ecshop68.com 涓昏鏄粰杩欎袱涓狪NPUT鍚勮嚜澧炲姞浜嗕竴涓狪D锛�id="goods_info_submit"  id="goods_info_reset" -->
		  ...
		  <!--淇敼浠ｇ爜_end By www.ecshop68.com-->
...
<script language="JavaScript">
  ...
  var marketPriceRate = <?php echo empty($this->_var['shop_ratio']) ? '1' : $this->_var['shop_ratio']; ?>;
...
?>
  ...
  function computePrice(inputName, rate, priceName)
  {
      ...
      shopPrice += "";      
      shopPrice = parseFloat(shopPrice).toFixed(2);
  ...
  {
    computePrice('market_price', 1/marketPriceRate);
...
?>
  ...
  {
    computePrice('shop_price', marketPriceRate, 'market_price');
    computePrice('integral', integralPercent / 100);
...
//澧炲姞璐у彿
function addGoodsSn(obj){
	var src      = obj.parentNode.parentNode;
	var tbl      = document.getElementById('tbody-huohao');
	...
	var cell = row.insertCell(-1);
	cell.align = 'center';
	cell.innerHTML = src.cells[0].innerHTML.replace(/(.*)(addGoodsSn)(.*)(\[\+\])([\s\S]*?)(spec_name\[\d*\])/gim, "$1 removeGoodsSn $3 \[\-\] $5 spec_name\["+rowl+"\]");
	cell.innerHTML = cell.innerHTML.replace(/<input(.*)(spec_price\[\d*\])/gi, "<input$1spec_price\["+rowl+"\]");
	cell.innerHTML = cell.innerHTML.replace(/<input(.*)(spec_no\[\d*\])/gi, "<input$1spec_no["+rowl+"]");
	cell.innerHTML = cell.innerHTML.replace(/<input(.*)(spec_city\[\d*\]\[\])/gi, "<input$1spec_city["+rowl+"][]");
	cell.innerHTML = cell.innerHTML.replace(/<input(.*)(default_show\[\d*\])/gi, "<input$1default_show["+rowl+"]");
	
	//清空新增加货号的值
	var spec_name = document.getElementsByName("spec_name["+(rowl)+"]");
	var spec_price = document.getElementsByName("spec_price["+(rowl)+"]");
	var spec_no = document.getElementsByName("spec_no["+(rowl)+"]");
	var default_show = document.getElementsByName("default_show["+(rowl)+"]");
	//alert(goodsSn_list[0].value)
	spec_name[0].value =  spec_price[0].value = spec_no[0].value = "";
	default_show[0].checked = false;
}
//鍒犻櫎璐у彿琛�
function removeGoodsSn(obj){
	var row = rowindex(obj.parentNode.parentNode);
	var tbl = document.getElementById('tbody-huohao');
	tbl.deleteRow(row);
}

function checkGoodsSnData(isSubmit,validator, num){
	var goodsSn = document.getElementsByName("goods_sn["+(num-1)+"]");
	var numErrNum = 0;
	for (i = 0 ; i < goodsSn.length ; i ++){
		if ((isSubmit != 1 || goodsSn.length > 1) && numErrNum <= 0 && goodsSn.item(i).value == ""){
			validator.addErrorMsg('鍟嗗搧璐у彿涓嶈兘涓虹┖');
			numErrNum++;
		}
	}
}
...
// 城市显示
function shwoRegion(obj){
	var res = obj.parentNode.parentNode;
	var node = res.nextSibling;
	if(node.nodeType!=1){
		node = res.nextSibling.nextSibling;
	}
	if(node.style.display == 'table-row'){
		node.style.display = 'none';
	}else{
		node.style.display = 'table-row';
	}
	//node.style.display= 'none';
	//res.innerHTML = '123546';
}
</script>
