...
elseif ($_REQUEST['act'] == 'insert')
{
	...
	$is_supplier = empty($_POST['is_supplier']) ? 0 : intval($_POST['is_supplier']);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_name, u.sex, u.is_supplier, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone".
    ...
    $sql = "SELECT u.user_id, u.is_supplier, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn,
    ...
    if ($row)
    {
		...
		$user['is_supplier']        = $row['is_supplier'];
        $user['sex']            = $row['sex'];
...
//导出会员信息
	...
	set_time_limit(0);
	$is_stop  = $_REQUEST['stop'];
	/*$int_num  = 100;//每次导出100条
	$int_page = $_REQUEST['page'] > 0 ? $_REQUEST['page'] : 1;
	$int_start = ($int_page - 1) * $int_num;
	$arr_user = $db->getAll('SELECT user_id, user_name FROM '.$ecs->table('users').' ORDER BY user_id ASC LIMIT '.$int_start.', '.$int_num);
	*/

	
	$arr_user = array();
	if ($_POST['checkboxes']){
		$arr_user = $db->getAll('SELECT user_id, user_name FROM '.$ecs->table('users')." WHERE user_id " . db_create_in($_POST['checkboxes']).' ORDER BY user_id ASC');
	}else{
		$arr_user = $db->getAll('SELECT user_id, user_name FROM '.$ecs->table('users').' ORDER BY user_id DESC LIMIT 10000');
	}

	foreach ($arr_user as $key=>$var){
		$arr_order = $db->getRow('SELECT order_id, tel, mobile, add_time FROM '.$ecs->table('order_info')." WHERE user_id = '".intval($var['user_id'])."' ORDER BY add_time DESC LIMIT 1");
		$var['mobile']   = $arr_order['mobile']? $arr_order['mobile'] : '';
		$var['tel']      = $arr_order['tel']? $arr_order['tel'] : '';
		$var['add_time'] = $arr_order['add_time']? local_date('Y-m-d H:i', $arr_order['add_time']) : '';
		$arr_user[$key] = $var;
	}

	$filename='会员信息_'.local_date('YmdHis');
	$filename = ecs_iconv('UTF8', 'GB2312',$filename);
	header("Content-type: application/vnd.ms-excel; charset=gbk");
	header("Content-Disposition: attachment; filename=$filename.xls");

	echo '<table border=1>';
		echo '<tr><td colspan=4 align=center>'.ecs_iconv('UTF8', 'GB2312', '会员信息').'</td></tr>';
		echo '<tr><td colspan=4>&nbsp;</td></tr>';
		echo '<tr><td align=center>'.
			ecs_iconv('UTF8', 'GB2312', '会员ID').'</td><td align=center>'.
			ecs_iconv('UTF8', 'GB2312', '会员卡号').'</td><td align=center>'.
			ecs_iconv('UTF8', 'GB2312', '会员手机').'</td><td align=center>'.
			ecs_iconv('UTF8', 'GB2312', '会员固话').'</td><td align=center>'.
			ecs_iconv('UTF8', 'GB2312', '最后一次下单时间').'</td></tr>';
	foreach ($arr_user as $key=>$user){
		echo '<tr><td>'.ecs_iconv('UTF8', 'GB2312', $user['user_id']).'</td><td>'.
				ecs_iconv('UTF8', 'GB2312', $user['user_name']).'</td><td>'.
				ecs_iconv('UTF8', 'GB2312', $user['mobile']).'</td><td>'.
				ecs_iconv('UTF8', 'GB2312', $user['tel']).'</td><td>'.
				ecs_iconv('UTF8', 'GB2312', $user['add_time']).'</td></tr>';
	}
	echo '</table>';
	exit;
