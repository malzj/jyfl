...
?>
  ...
  <tr>
      <td colspan="2">
      <input type="hidden" name="act" id="act" value="batch_remove" />
      <input type="submit" id="btnSubmit" value="<?php echo $this->_var['lang']['button_remove']; ?>" disabled="true" class="button" onclick="document.getElementById('act').value='batch_remove';" />
	  <input type="submit" id="btnSubmit_e" value="全部导出" class="button" onclick="this.form.target = '_self';document.getElementById('act').value='export';" />
	  </td>
...
function confirm_bath()
{
  ...
  if (document.getElementById('act').value == 'export'){
		if (check == false){
			return confirm('由于您没有选择要导出的会员，本次只导出最近10000条会员数据\n确定导出吗？');
		}else{
			return true;
		}
  }
