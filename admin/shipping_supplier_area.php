...
$exc = new exchange($ecs->table('supplier_shipping_area'), $db, 'shipping_area_id', 'shipping_area_name');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('href'=>'shipping_supplier_area.php?act=add&shipping='.$shipping_id,
        'text' => '新建供货商配送区域'));
    $smarty->assign('full_page', 1);

    assign_query_info();
    $smarty->display('shipping_supplier_area_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query'){
	...
	$sort_flag  = sort_flag($list['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
	make_json_result($smarty->fetch('shipping_supplier_area_list.htm'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
...
elseif ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping']))
{
	...
	$order_shipping_fee = array(array('order_amount'=>'', 'shipping_fee'=>''));
	$smarty->assign('order_shipping_fee',  $order_shipping_fee);

	$arr_suppliers = $db->getAll('SELECT supplier_id, supplier_name FROM '.$ecs->table('supplier')." WHERE status = 1 ORDER BY supplier_id ASC");
	$smarty->assign('suppliers',        $arr_suppliers);
	
	$smarty->assign('is_shipping_times','1');
	
    assign_query_info();
    $smarty->display('shipping_supplier_area_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$ecs->table("supplier_shipping_area").
    ...
    else
    {
		...
		if (isset($_POST['order_amount'])){
			$arr_order_shipping_fee = array();
			foreach ($_POST['order_amount'] as $key=>$var){
				if (!empty($var)){
					$arr_order_shipping_fee[$key]['order_amount'] = $var;
					$arr_order_shipping_fee[$key]['shipping_fee'] = $_POST['shipping_fee'][$key];
				}
			}
			if ($arr_order_shipping_fee){
				$count++;
				$config[$count]['name']  = 'order_shipping_fee';
				$config[$count]['value'] = $arr_order_shipping_fee;
			}
		}

		// 处理配送时间
		$shipping_config = array();
		foreach( array('shipping_start', 'shipping_end', 'shipping_waiting','shipping_booking') as $shiping){
			$shipping_config[$shiping] = $_POST[$shiping];
		}
		
        $sql = "INSERT INTO " .$ecs->table('supplier_shipping_area').
                " (supplier_id, shipping_area_name, shipping_id, configure, shipping_times) ".
                "VALUES".
                " ('$_POST[supplier_id]', '$_POST[shipping_area_name]', '$_POST[shipping]', '" .serialize($config). "', '".serialize($shipping_config)."')";
        ...
        if (isset($_POST['regions']) && is_array($_POST['regions']))
        {
            ...
            {
                $sql = "INSERT INTO ".$ecs->table('supplier_area_region')." (shipping_area_id, region_id) VALUES ('$new_id', '$val')";
                $db->query($sql);
            }
        }
        ...
        $lnk[] = array('text' => $_LANG['back_list'], 'href'=>'shipping_supplier_area.php?act=list&shipping='.$_POST['shipping']);
        $lnk[] = array('text' => $_LANG['add_continue'], 'href'=>'shipping_supplier_area.php?act=add&shipping='.$_POST['shipping']);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* ".
            "FROM " .$ecs->table('shipping'). " AS a, " .$ecs->table('supplier_shipping_area'). " AS b ".
...
//       }
       ...
       if ($val['name'] == 'fee_compute_mode')
       {
           ...
           unset($fields[$key]);
       }else if ($val['name'] == 'order_shipping_fee'){
		   $order_shipping_fee = $val['value'];
		   unset($fields[$key]);
	   }
       else
       {
	...
	$order_shipping_fee = $order_shipping_fee ? $order_shipping_fee : array(array('order_amount'=>'', 'shipping_fee'=>''));
	$smarty->assign('order_shipping_fee',$order_shipping_fee);

	/* 处理配送时间 */
	$shipping_times = unserialize($row['shipping_times']);
	$open_time = checkSupplierOpenTime($row['supplier_id']);
	if($open_time == 1 )
	{
		$smarty->assign('is_shipping_times','1');
		$smarty->assign('shipping_times',$shipping_times);
	}
	else
	{
		$smarty->assign('is_shipping_times','0');
	}
    ...
    $sql = "SELECT a.region_id, r.region_name ".
            "FROM ".$ecs->table('supplier_area_region')." AS a, ".$ecs->table('region'). " AS r ".
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    else
    {
        ...
        $sql = "UPDATE " .$ecs->table('supplier_shipping_area').
                " SET shipping_area_name='$_POST[shipping_area_name]', ".
                    "configure='" .serialize($config). "',  shipping_times='".serialize($shipping_config)."' ".
        ...
        /* 清除原有的城市和地区 */
        $db->query("DELETE FROM ".$ecs->table("supplier_area_region")." WHERE shipping_area_id='$_POST[id]'");

        /* 添加选定的城市和地区 */
        foreach ($selected_regions AS $key => $val)
        {
            $sql = "INSERT INTO ".$ecs->table('supplier_area_region')." (shipping_area_id, region_id) VALUES ('$_POST[id]', '$val')";
            $db->query($sql);
        }
...
elseif ($_REQUEST['act'] == 'multi_remove')
{
    ...
    if (isset($_POST['areas']) && count($_POST['areas']) > 0)
    {
        ...
        {
			$query = $db->query("DELETE FROM " .$ecs->table('supplier_shipping_area'). " WHERE shipping_area_id='$v'");
			if ($query){
				$db->query("DELETE FROM " .$ecs->table('supplier_area_region'). " WHERE shipping_area_id='$v'");
			}
            $i++;
        }
    ...
    /* 返回 */
    $links[0] = array('href'=>'shipping_supplier_area.php?act=list&shipping=' . intval($_REQUEST['shipping']), 'text' => $_LANG['go_back']);
...
elseif ($_REQUEST['act'] == 'remove_area')
{
    ...
    $exc->drop($id);
    $db->query('DELETE FROM '.$ecs->table('supplier_area_region').' WHERE shipping_area_id='.$id);
    ...
    make_json_result($smarty->fetch('shipping_supplier_area_list.htm'));
}

elseif ($_REQUEST['act'] == 'supplier_shipping_check'){
	
	$supplier_id = isset($_GET['sid']) ? intval($_GET['sid']) : 0 ;
	...
	$sql =  "SELECT open_time FROM " .$GLOBALS['ecs']->table('supplier') .
			"WHERE supplier_id = ".$supplier_id;
	
	if($row = $GLOBALS['db']->getOne($sql))
	{
		echo $row;
	}
	else
 ...
 *  检查供应商是否启用配送时间
 */
function checkSupplierOpenTime($sid){
	$sql =  "SELECT open_time FROM ".$GLOBALS['ecs']->table('supplier').
...
function get_shipping_area_list($shipping_id)
{
	...
	if ($result === false){
		...
		$filter['shipping'] = !empty($_REQUEST['shipping']) ? intval($_REQUEST['shipping']) : 0;
		$filter['supplier'] = !empty($_REQUEST['supplier']) ? trim($_REQUEST['supplier']) : '';
		$where = '1';

		if ($filter['shipping']){
			$where .= ' AND a.shipping_id = '.$filter['shipping'];
		}
		if (!empty($filter['supplier'])){
			$where .= " AND s.supplier_name LIKE '%".$filter['supplier']."%'";
		}

		$str_sql = 'SELECT COUNT(*) FROM '.$GLOBALS['ecs']->table('supplier_shipping_area') . ' AS a LEFT JOIN '.$GLOBALS['ecs']->table('supplier')." as s ON s.supplier_id = a.supplier_id WHERE $where";
		$filter['record_count'] = $GLOBALS['db']->getOne($str_sql);
		$filter['sort_by']      = empty($_REQUEST['sort_by'])    ? 'a.supplier_id' : trim($_REQUEST['sort_by']);
		$filter['sort_order']   = empty($_REQUEST['sort_order']) ? 'ASC'    : trim($_REQUEST['sort_order']);
		$filter = page_and_size($filter);
		$str_sql = 'SELECT a.*, s.supplier_name FROM '.$GLOBALS['ecs']->table('supplier_shipping_area').' as a LEFT JOIN '.$GLOBALS['ecs']->table('supplier')." as s ON s.supplier_id = a.supplier_id WHERE $where ORDER BY $filter[sort_by] $filter[sort_order]";
	...
	while ($row = $GLOBALS['db']->fetchRow($res)){
		...
		$sql = "SELECT r.region_name, r.region_id, r.parent_id, r.region_type " .
				"FROM " . $GLOBALS['ecs']->table('supplier_area_region'). " AS a, " .
					$GLOBALS['ecs']->table('region') . " AS r ".
				"WHERE a.region_id = r.region_id ".
				"AND a.shipping_area_id = '$row[shipping_area_id]'";
		...
		$regions = '';
		while ($region = $GLOBALS['db']->fetch_array($query)){
			if ($region['region_type'] == 2){
				$str_prname = $GLOBALS['db']->getOne('SELECT region_name FROM '.$GLOBALS['ecs']->table('region')." WHERE region_id = '".intval($region['parent_id'])."'");
			}else{
				$str_prname = '';
			}
			$region['name'] = !empty($str_prname) ? $str_prname .'-'. $region['region_name'] : $region['region_name'];
			if (empty($regions)){
				$regions = $region['name'];
			}else{
				$regions .= ', '.$region['name'];
	...
	return array('area' => $list, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
	return $list;
}
