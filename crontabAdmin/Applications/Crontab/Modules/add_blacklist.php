...
function add_blacklist($file = '')
{
    ...
    if(!empty($file))
    {
        $forbidden_dir = WEB_ROOT.'/Crontab/'.basename(Crontab\Config::$forbidden_dir);
        $cron_dir = WEB_ROOT.'/Crontab/'.basename(Crontab\Config::$cron_dir);
        $run_dir = WEB_ROOT.'/Crontab/'.basename(Crontab\Config::$run_dir);
        $pid_dir = WEB_ROOT.'/Crontab/'.basename(Crontab\Config::$pid_dir);
        $lock_dir = WEB_ROOT.'/Crontab/'.basename(Crontab\Config::$lock_dir);
