...
elseif ($_REQUEST['act'] == 'insert')
{
	...
	$listorder 	= !empty($_POST['listorder'])? trim($_POST['listorder']) : 0; 
	$bgcolor 	= !empty($_POST['bgcolor'])  ? trim($_POST['bgcolor']) : 0;
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $listorder = !empty($_POST['listorder']) ? intval($_POST['listorder']) : 0;
    $bgcolor 	= !empty($_POST['bgcolor'])  ? trim($_POST['bgcolor']) : 0;
...
elseif($_REQUEST['act'] == 'edit_listorder'){
	check_authz_json('ad_manage');
	
	$id        = intval($_POST['id']);
	$listorder = json_str_iconv(trim($_POST['val']));	
	
	if ($exc->edit("listorder = '$listorder'", $id))
	{
		admin_log($ad_name,'edit','ads');
		make_json_result(stripslashes($listorder));
	}
	else
	{
		make_json_error($db->error());
