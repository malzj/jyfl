...
if ($action == 'saveOrder')
{
    ...
    {
        show_message('您最多可以购买 '.$good['MaxQuantityEachOrder'].' 张！');
    }
...
{
    $secrets = findData( 'huanlegu_order', 'is_pay=0 AND user_id = "'.$_SESSION['user_id'].'" AND secret="'.$secret.'"' , 'id');
    if ($secrets[0]['id'])
        return $secrets[0]['id'];
    else
        return false;
...
$smarty->display('huanlegu/huanlegu_goods_order.dwt');
