 ...
 * @return  void
 */
function order_paid($log_id, $pay_status = PS_PAYED, $note = '', $pay_no = '', $api_order_id=0)
{
    /* 取得支付编号 */
            ...
            elseif ($pay_log['order_type'] == PAY_SURPLUS)
            {
                ...
                if(empty($res_id))
                {
                    ...
                    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('user_account') .
                           " SET paid_time = '" .gmtime(). "', is_paid = 1, pay_no = " .$pay_no.
                           " WHERE id = '$pay_log[order_id]' LIMIT 1";
                    $GLOBALS['db']->query($sql);

                    /* 取得添加预付款的用户以及金额 */
                    $sql = "SELECT user_id, order_sn, amount FROM " . $GLOBALS['ecs']->table('user_account') .
