...
//        }
        ...
        if ($iswap == 1){
        	...
        	$button = '<div style="text-align:center"><input type="button" onclick="window.open(\'https://www.alipay.com/cooperate/gateway.do?'.$param. '&sign='.md5($sign).'&sign_type=MD5\')" value="立即支付" class="bnt_blue_1" /></div>';
        }
        return $button;
    }
    ...
    function respond()
    {
        ...
        // 通过支付信息得到卡号
        	...
        	$pay_log = $GLOBALS['db']->getOne("SELECT order_id FROM " . $GLOBALS['ecs']->table('pay_log') ." WHERE log_id = '$order_sn'");
        ...
        {
            /* 改变订单状态 */
            order_paid($order_sn, 2, '', $_GET['trade_no']);
            return true;
        }
        else
        {
