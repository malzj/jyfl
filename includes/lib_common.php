...
function clear_tpl_files($is_cache = true, $ext = '')
{
	...
	delDir(ROOT_PATH . $tmp_dir . '/films_caches/');
    ...
    switch ($app)
    {
        ...
        case 'coupons_list':
           if (empty($gid))
           {
            ...
            {
            	$uri = $rewrite ? 'coupons_list-' . $gid : 'coupons_list.php?id=' . $gid;
            }
 ...
 * @return  void
 */
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER, $is_api = 0, $order = '')
{
	...
	{
		$card_id = $GLOBALS['db']->getOne('SELECT card_id FROM '.$GLOBALS['ecs']->table('users')." where user_id = ".$user_id);
	}
	else {
	...
	{
		$pay_price = $GLOBALS['db']->getOne('SELECT pay_than FROM '.$GLOBALS['ecs']->table('card_rule')." where id = ".$card_id);
	...
	// 实际充值点数
	$int_sjMoney = $user_money / $pay_than;
    ...
    if (!empty($is_api)){
    	...
    	if (!empty($order['order_sn']))
    	{
			...
			$str_username = $GLOBALS['db']->getOne('SELECT user_name FROM '.$GLOBALS['ecs']->table('users')." WHERE user_id = '$user_id'");
			if ($user_money > 0 && $is_api == 1){
				//审核充值操作
				$arr_param = array(
    	...
    	{
    		// 卡密码和卡号
    		$userinfo = $GLOBALS['db']->getRow('SELECT user_name, password FROM '.$GLOBALS['ecs']->table('users')." WHERE user_id = '".$user_id."'");
	...
	$sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
			...
			" card_money = card_money + ('$int_sjMoney')," .
	...
	{
	    $_SESSION['BalanceCash'] += $int_sjMoney;
	}
...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
    ...
    //取得商品促销价格列表
    /* 取得商品信息 */
    $sql = "SELECT g.promote_price, g.promote_start_date, g.promote_end_date, g.cat_id,g.supplier_id, g.market_price, ".
    ...
    // TODO 卡规则折扣  author： guoyunpeng
   /*  $ratio = get_card_rule_ratio($goods['cat_id']);
    if ($ratio !== false)
    {
    	$final_price = $goods['market_price'];
    } */
    // 加入商品规格
    //如果需要加入规格价格
    if ($is_spec_price)
    {
        /*  $final_price = $goods['market_price'];
        // 如果不在卡规则里，商品价格 = （市场价+属性价格） * 商城比例
        $supplier_row = $GLOBALS['db']->getRow("SELECT shop_ratio,supplier_name FROM ".$GLOBALS['ecs']->table('supplier')." WHERE supplier_id = ".$goods['supplier_id']);  
       	// TODO 易果生鲜，走商城售价
       	if ($supplier_row['supplier_name'] == '易果生鲜')
       	{
		...
		/* if (floatval($supplier_row['shop_ratio']) > 0 && $ratio === false)
		{
			$final_price = price_format($final_price * $supplier_row['shop_ratio']);
		} */
    }
    ...
    /* if ($ratio !== false)
    {
    	$final_price = price_format($final_price * $ratio);
    } */
...
function get_add_cn($address_id){
	$sql = "SELECT region_name FROM " . $GLOBALS['ecs']->table('region') .
			" WHERE region_id = '$address_id'";
	return $GLOBALS['db']->getOne($sql);
}

//返回指定地区指定商品货号
function getGoodsSn($int_goodsId, $int_cityId = 0){
	if (empty($int_goodsId)){
		return '';
	}
	$int_cityId = $int_cityId ? $int_cityId : $GLOBALS['int_cityId'];
	return $GLOBALS['db']->getOne('SELECT goods_sn FROM '. $GLOBALS['ecs']->table('goods_huohao') ." WHERE goods_id = '$int_goodsId' AND FIND_IN_SET('$int_cityId', region_ids)");
}

function getAreaNo($int_cityId = 0, $type=''){
	$int_cityId = $int_cityId ? $int_cityId : intval($GLOBALS['int_cityId']);
	switch ($type)
	{
	    case 'yanchu':     $str_select = 'yanchu_id'; break;
	    case 'komovie':    $str_select = 'komovie_id'; break;
	    case 'venues':     $str_select = 'dongwang_id'; break;
	    default:           $str_select = 'dianying_id'; break;
	}
	return (int) $GLOBALS['db']->getOne('SELECT '.$str_select.' FROM '.$GLOBALS['ecs']->table('region').' WHERE region_id = '.$int_cityId);

}
function array2url($arr=array()){
    $str = '';
...
/* 设置会员对应的卡规则  */
function set_card_id( $username, $card_id)
{
	// 用户为空，返回
	if (empty( $username ))
	{
	...
	$cardid = $GLOBALS['db']->getOne("SELECT card_id FROM ".$GLOBALS['ecs']->table('users'). " WHERE user_name = '".$username."'");
	...
	// 当前卡不在卡规则里，卡规则id设置成 -2
	if (empty($card_id) && $cardid == '-2' )
	{
		return false;
	}
	if(empty($card_id))
	...
	return $GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('users')." SET card_id = ".$card_id." WHERE user_name = '".$username."'");
}
