...
elseif ($_REQUEST['act'] == 'grabState')
{
    ...
    $order = $db->getRow($sql);
	$order['platform']=!empty($order['platform'])?$order['platform']:'抠电影';
	if(PLATFORM_KOMOVIE == $order['platform']){
		...
		$res_status = $wpwMovie -> sellSearchOrderInfoBySID($order['order_sn']);
		switch($res_status['Data'][0]['PayFlag'])
		{
			case -1:
				$orderStatus = 2;
				break;
...
elseif( $_REQUEST['act'] == 'order_msg')
{
	...
	$sql = " SELECT order_sn,platform FROM ".$GLOBALS['ecs']->table('seats_order')." WHERE id = ".$id;
	...
	if(PLATFORM_KOMOVIE == $order['platform']) {
		...
		$wpwMovie = new wpwMovie();
		$res_status = $wpwMovie -> sellSearchOrderInfoBySID($order['order_sn']);
		if($res_status['ErrNo']==0){
			$order_msg = $res_status['Data'][0]['TicketInfo'];
		}
	}
