    ...
    <!--<?php else: ?>-->
        <!--<?php if ($this->_var['field']['name'] != 'item_fee'): ?>-->
            <tr id="<?php echo $this->_var['field']['name']; ?>">
              <td class="label"><?php echo $this->_var['field']['label']; ?></td>
              <td>
				<input type="text" name="<?php echo $this->_var['field']['name']; ?>"  maxlength="60" size="20" value="<?php echo $this->_var['field']['value']; ?>" /><?php echo $this->_var['lang']['require_field']; ?>
				<?php if ($this->_var['field']['name'] == free_money): ?><br><font color=red>注：商品订单商品金额达到该值将免运费</font><?php endif; ?>
			</td>
            </tr>
        <!--<?php else: ?>-->
            <tr id="<?php echo $this->_var['field']['name']; ?>" style="display:none">
	...
	<tr>
		...
		<td valign="top">
			<table id="order_shiping_fee" cellspacing="0" border="0" style="border:none;" width="90%">
				<?php $_from = $this->_var['order_shipping_fee']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'order_shipping');$this->_foreach['order_shipping'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['order_shipping']['total'] > 0):
    foreach ($_from AS $this->_var['order_shipping']):
        $this->_foreach['order_shipping']['iteration']++;
?>
				<tr>
					<td valign="top">
						<?php if ($this->_foreach['order_shipping']['iteration'] == 1): ?>
						<a href="javascript:;" onclick="addOrderShiping(this)">[+]</a>
						<?php else: ?>
						<a href="javascript:;" onclick="removeOrderShiping(this)">[−]</a>
						<?php endif; ?>
						订单商品金额达到：<input type="text" name="order_amount[]" value="<?php echo $this->_var['order_shipping']['order_amount']; ?>" />&nbsp;&nbsp;运费金额：<input type="text" name="shipping_fee[]" value="<?php echo $this->_var['order_shipping']['shipping_fee']; ?>" />
					</td>
				</tr>
				<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
			</table>
			<table id="orader_shiping_fee" cellpadding="0" cellspacing="0" border="0" style="border:none;" width="90%">
				<tr>
					<td valign="top">
						<font color=red>注：当免费额度大于0时，订单商品金额只有小于免费额度此项设置才生效</font>
					</td>
				</tr>
			</table>
			<br>
