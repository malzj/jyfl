...
function admin_log($sn = '', $action, $content)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('admin_log') . ' (log_time, user_id, log_info, ip_address, is_supplier) ' .
            " VALUES ('" . gmtime() . "', '$_SESSION[supplier_id]', '" . stripslashes($log_info) . "', '" . real_ip() . "', '1')";
 ...
 */
function suppliers_ratio($cols='shop_ratio', $sid=0){
	...
	/* 查询 */
	//$suppliers_list = suppliers_list_info(' is_check = 1 ');
	$sql = "SELECT supplier_id, cost_ratio, shop_ratio FROM " . $GLOBALS['ecs']->table("supplier") ." WHERE status =1";
	$suppliers_list = $GLOBALS['db']->getAll($sql);
	
	//return $suppliers;
	$shop_ratio = 1;
	foreach($suppliers_list as $suppliers){
		if ($suppliers['supplier_id'] == $sid && $suppliers[$cols] > 0)
		{
			$shop_ratio = $suppliers[$cols];
		}
	}
...
function create_shop_settiongs()
{
	...
	if($num>0){
		...
		$insql = "INSERT INTO ". $ecs->table('supplier_shop_config') ." (`id`, `parent_id`, `code`, `type`, `store_range`, `store_dir`, `value`, `sort_order`, `supplier_id`) VALUES
				...
				(125, 1, 'shop_closed', 'select', '0,1', '', '1', 1, ".$_SESSION['supplier_id']."),
