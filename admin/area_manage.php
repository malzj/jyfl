...
elseif ($_REQUEST['act'] == 'add_area')
{
	...
	$region_english    = json_str_iconv(trim($_POST['region_english']));
    $region_type    = intval($_POST['region_type']);
	$komovie_id    = intval($_POST['komovie_id']);
	$yanchu_id    = intval($_POST['yanchu_id']);
    ...
    $sql = "INSERT INTO " . $ecs->table('region') . " (parent_id, region_name, region_english, region_type,komovie_id,yanchu_id) ".
           "VALUES ('$parent_id', '$region_name', '$region_english', '$region_type', '$komovie_id', '$yanchu_id')";
...
elseif ($_REQUEST['act'] == 'edit_area_englishname')
{
    check_authz_json('area_manage');

    $id = intval($_POST['id']);
    $region_english = json_str_iconv(trim($_POST['val']));

    if (empty($region_english))
    {
        make_json_error($_LANG['region_name_empty']);
    }
    ...
    /* 查看区域是否重复 */
    $parent_id = $exc->get_name($id, 'parent_id');
    if (!$exc->is_only('region_english', $region_english, $id, "parent_id = '$parent_id'"))
    {
        make_json_error($_LANG['region_name_exist']);
    }

    if ($exc->edit("region_english = '$region_english'", $id))
    {
        admin_log($region_english, 'edit', 'area');
        make_json_result(stripslashes($region_english));
    }
    else
    {
        make_json_error($db->error());
...
elseif ($_REQUEST['act'] == 'komovie_id')
{
    check_authz_json('area_manage');

    $id = intval($_POST['id']);
    $komovie_id = json_str_iconv(trim($_POST['val']));
    ...
    /* 查看区域是否重复 */
    $parent_id = $exc->get_name($id, 'parent_id');
    if (!$exc->is_only('komovie_id', $komovie_id, $id, "parent_id = '$parent_id'"))
    {
        make_json_error($_LANG['region_name_exist']);
    }

    if ($exc->edit("komovie_id = '$komovie_id'", $id))
    {
        admin_log($komovie_id, 'edit', 'area');
        make_json_result(stripslashes($komovie_id));
    }
    else
    {
        make_json_error($db->error());
...
elseif ($_REQUEST['act'] == 'yanchu_id')
{
    check_authz_json('area_manage');

    $id = intval($_POST['id']);
    $yanchu_id = json_str_iconv(trim($_POST['val']));
    ...
    /* 查看区域是否重复 */
    $parent_id = $exc->get_name($id, 'parent_id');
    if (!$exc->is_only('yanchu_id', $yanchu_id, $id, "parent_id = '$parent_id'"))
    {
        make_json_error($_LANG['region_name_exist']);
    }

    if ($exc->edit("yanchu_id = '$yanchu_id'", $id))
    {
        admin_log($yanchu_id, 'edit', 'area');
        make_json_result(stripslashes($yanchu_id));
    }
    else
    {
        make_json_error($db->error());
