...
//-- 退货单列表
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'card_list')
{
    /* 检查权限 */
    admin_priv('back_view');

    /* 查询 */
    $result = user_card_list();
...
//-- 卡合并，搜索、排序、分页
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'user_back_query')
{
	/* 检查权限 */
	admin_priv('back_view');
	...
	$sort_flag = sort_flag($result['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
	make_json_result($smarty->fetch('user_card_list.htm'), '', array('filter' => $result['filter'], 'page_count' => $result['page_count']));
}
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['return']))
    {
        ...
        $order          = order_info($order_id);
		$arr_orderGoods = order_goods1($order_id);
		...
		$smarty->assign('show_refund', $show_refund);
		$smarty->assign('ordergoods', $arr_orderGoods);
		$smarty->assign('order', $order);
		$smarty->display('order_return.htm');
		exit;
    }
    /* 指派 */
	...
	elseif (isset($_REQUEST['remove_back1']))
    {
        $log_id = $_REQUEST['log_id'];
        /* 删除退货单 */
        if(is_array($log_id))
        {
        foreach ($log_id as $value_is)
            {
                $sql = "DELETE FROM ".$ecs->table('card_log'). " WHERE log_id = '$value_is'";
                $db->query($sql);
            }
        }
        else
        {
            $sql = "DELETE FROM ".$ecs->table('card_log'). " WHERE log_id = '$log_id'";
            $db->query($sql);
        }
        /* 返回 */
        sys_msg($_LANG['tips_back_del'], 0, array(array('href'=>'order.php?act=card_list' , 'text' => $_LANG['return_list'])));
    }
    /* 批量打印订单 */
	...
	//一键发货
		...
		if (!empty($invoice_no)){
			...
			$delivery['supplier_id'] = intval($order['supplier_id']);
			...
			$filter_fileds = array(
				...
				'suppliers_id', 'status', 'order_id', 'shipping_name', 'supplier_id'
			);
			$_delivery = array();
			...
			if ($order_finish)
			{
				...
				if ($GLOBALS['_CFG']['sms_order_shipped'] == '1'){
					...
					$content = '您的订单已发货，订单号为'.$order['order_sn'].'收货人为'.$order['consignee'].'收货地址为'.$order['address'].'，请注意查收【华影商城】';
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('return' == $operation)
    {
		...
		$flo_money = $_REQUEST['refund_money'];
        if($flo_money<=0){
            sys_msg('退款金额不能小于0');
        }
        if ($flo_money > $order['total_fee']){
            sys_msg('退款金额大于订单支付金额，请重新操作');
        }
		$discount = $order['discount'] + $flo_money;
		// $arr_gids  = $_REQUEST['gids'];
		// $int_count = (int)$db->getOne('SELECT COUNT(*) FROM '.$ecs->table('order_goods')." WHERE order_id ='$order_id'");

		if ($discount==$order['money_paid']){
			$return_status = 2;
        ...
        {
			$order['back_sn'] = get_order_sn();
            $refund_type = $_REQUEST['refund'];
            $refund_note = $_REQUEST['refund_note'];
            //order_refund($order, $refund_type, $refund_note, $flo_money);
			//退款金额大于0时
			if ($flo_money > 0){
				...
				$arr_param = array(
						...
						'TransationInfo' => array( 'TransRequestPoints'=>$flo_money)
				);
		...
		$arr = array(
			...
			'refund_money'     => $flo_money
		);
		update_order($order_id, $arr);
        /* 记录log */
        order_action($order['order_sn'], $order_status, SS_RECEIVED, PS_PAYED, $action_note);
        ...
        /* 如果使用库存，则增加库存（不论何时减库存都需要） 且是全部退款 */
        if ($_CFG['use_storage'] == '1' && $order_status == OS_RETURNED)
        {
		...
		//标记订单商品为已退货，增加商品库存
		foreach ($arr_gids as $goods_id){
			if (!empty($goods_id)){
				$db->query('UPDATE '.$ecs->table('order_goods')." SET is_refund = 1 WHERE goods_id = '$goods_id' and order_id = 'order_id'");
			
				if ($_CFG['use_storage'] == '1' && $order_status == OS_RETURNED_PART){
					//发货数量
					$int_num = (int)$db->getOne('SELECT send_number FROM '.$ecs->table('delivery_goods')." WHERE goods_id = '$goods_id'");
					//修改商品库存
					change_goods_storage($goods_id, 0, $int_num);
				}
			}
		}
		...
		/* 所有红包删除 */
		$sql = "DELETE FROM " . $GLOBALS['ecs']->table('user_bonus') . " WHERE huoqu_orderid = '$order_id'  AND order_id = 0";
        ...
        if ($delivery_list)
        {
            foreach ($delivery_list as $list)
            {
                ...
                $sql_back .= " ( '" . $list['delivery_sn'] . "', '" . $list['order_sn'] . "',
                              ...
                              '" . $list['mobile'] . "', '" . $list['best_time'] . "', '" . $list['postscript']. ($_REQUEST['refund_note'] ? "\n".$_REQUEST['refund_note'] : '') . "',
                              '" . $list['how_oos'] . "', '" . $list['insure_fee'] . "',
                              '" . $list['shipping_fee'] . "', '" . $list['update_time'] . "',
                              '" . $list['suppliers_id'] . "', '" . GMTIME_UTC . "',
                              '" . $list['agency_id'] . "', '" . $list['invoice_no'] . "', '$flo_money', '".$order['supplier_id']."'
                              )";
                ...
                $sql_back_goods = "INSERT INTO " . $ecs->table('back_goods') . " (back_id, goods_id, product_id, product_sn, goods_name,goods_sn, is_real, send_number, goods_attr)
                                   ...
                                   FROM " . $ecs->table('delivery_goods') . "
                                   WHERE delivery_id = " . $list['delivery_id']." AND goods_id ".db_create_in($arr_gids);
       ...
       $sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') . "
        ...
        $sql = "UPDATE " . $GLOBALS['ecs']->table('order_goods') . "
                SET send_number = 0
                WHERE order_id = '$order_id' AND goods_id ".db_create_in($arr_gids);
        $GLOBALS['db']->query($sql, 'SILENT');
...
function order_list()
{
    ...
    if ($result === false)
    {
		...
		$filter['supplier_id'] = isset($_REQUEST['supplier_id']) ? intval($_REQUEST['supplier_id']) : -1;
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.return_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.address,o.source, o.email, o.tel, o.extension_code, o.extension_id, " .
        ...
        //error_log(var_export($sql,true),'3','error.log');
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT delivery_id, delivery_sn, order_sn, order_id, add_time, action_user, consignee, country,
                       province, city, district, tel, status, update_time, email, suppliers_id, supplier_id
                FROM " . $GLOBALS['ecs']->table("delivery_order") . "
                $where
                ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order']. "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
    ...
    foreach ($row AS $key => $value)
    {
		...
		$row[$key]['suppliers_name'] =  $GLOBALS['db']->getOne("SELECT supplier_name FROM " . $GLOBALS['ecs']->table('supplier') . " WHERE supplier_id = '$value[supplier_id]'");
    }
...
function back_list()
{
    ...
    if ($result === false)
    {
		...
		if ($_REQUEST['supp']){
			$where .= intval($_REQUEST['supp']) > 0 ? ' AND o.supplier_id > 0 ' : ' AND o.supplier_id = 0 ';
		}
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('back_order') .' as b LEFT JOIN '.$GLOBALS['ecs']->table('order_info').' as o ON o.order_id = b.order_id ' . $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        /* 查询 */
        $sql = "SELECT b.back_id, b.delivery_sn, b.order_sn, b.order_id, b.add_time, b.action_user, b.consignee, b.country,
                       b.province, b.city, b.district, b.tel, b.status, b.update_time, b.email, b.return_time, o.refund_money
                FROM " . $GLOBALS['ecs']->table("back_order") . " as b LEFT JOIN ". $GLOBALS['ecs']->table("order_info")." as o ON o.order_id = b.order_id
                $where
                ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order']. "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
...
function user_card_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 过滤信息 */
        $filter['card_from'] = empty($_REQUEST['card_from']) ? '' : trim($_REQUEST['card_from']);
        $filter['card_to'] = empty($_REQUEST['card_to']) ? '' : trim($_REQUEST['card_to']);
        ...
        if ($filter['card_from'])
        {
            $where .= " AND b.card_from LIKE '%" . mysql_like_quote($filter['card_from']) . "%'";
        }
        if ($filter['card_to'])
        {
            $where .= " AND b.card_to LIKE '%" . mysql_like_quote($filter['card_to']) . "%'";
        }
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('card_log') .' as b ' . $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        /* 查询 */
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table("card_log") . " as b $where
                ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order']. "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
...
function back_order_info($back_id)
{
    ...
    $sql = "SELECT b.*, o.refund_money FROM " . $GLOBALS['ecs']->table('back_order') . " as b LEFT JOIN ".$GLOBALS['ecs']->table('order_info')." as o ON o.order_id = b.order_id 
