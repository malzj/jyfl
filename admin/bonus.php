...
if ($_REQUEST['act'] == 'add')
{
	...
	$arr_supplier = $db->getAll('SELECT supplier_id, supplier_name FROM '.$ecs->table('supplier')." WHERE status = 1 ORDER BY supplier_id ASC");
	$smarty->assign('supplierlist',     $arr_supplier);
...
if ($_REQUEST['act'] == 'insert')
{
	...
	$bonus_name   = !empty($_POST['bonus_name']) ? trim($_POST['bonus_name']) : '';
    ...
    /* 插入数据库。 */
    /*$sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, bonus_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_goods_amount_fa,min_amount,min_goods_amount)
    VALUES ('$type_name', '$bonus_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
            '$use_startdate',
            '$use_enddate',
            '$_POST[send_type]',
			'$_POST[min_goods_amount_fa]',
            '$min_amount','" . floatval($_POST['min_goods_amount']) . "')";*/
	$sql = "INSERT INTO ".$ecs->table('bonus_type')." (type_name, bonus_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_goods_amount_fa,min_amount,min_goods_amount,supplier_id,endday,mmgz)
    VALUES ('$type_name',
			'$bonus_name',
            '$_POST[type_money]',
            '$send_startdate',
            ...
            '$_POST[send_type]',
			'$_POST[min_goods_amount_fa]',
            '$min_amount','" . floatval($_POST['min_goods_amount']) . "', '$_POST[supplier_id]', '$_POST[endday]', '$_POST[mmgz]')";
...
if ($_REQUEST['act'] == 'update')
{
    ...
    /*$sql = "UPDATE " .$ecs->table('bonus_type'). " SET ".
           ...
           "min_amount      = '$min_amount', " .
			"min_goods_amount_fa       = '$_POST[min_goods_amount_fa]', ".
...
if ($_REQUEST['act'] == 'send_by_print')
{
	...
	$bonus_type = bonus_type_info($bonus_typeid);
    ...
    for ($i = 0, $j = 0; $i < $bonus_sum; $i++)
    {
		...
		$bonus_pwd = getBonusPwd($bonus_type['mmgz']);
		$bonus_pwd = encrypt($bonus_pwd);
        //$db->query("INSERT INTO ".$ecs->table('user_bonus')." (bonus_type_id, bonus_sn) VALUES('$bonus_typeid', '$bonus_sn')");
		$db->query("INSERT INTO ".$ecs->table('user_bonus')." (bonus_type_id, bonus_sn, bonus_pwd) VALUES('$bonus_typeid', '$bonus_sn', '$bonus_pwd')");
...
if ($_REQUEST['act'] == 'gen_excel')
{
    ...
    if (EC_CHARSET != 'gbk')
    {
		...
		echo ecs_iconv('UTF8', 'GB2312', '所属供货商') ."\t";
	...
	$sql = "SELECT ub.bonus_id, ub.bonus_type_id, ub.bonus_sn, ub.bonus_pwd, bt.type_name, bt.type_money, bt.use_end_date, bt.supplier_id ".
           "FROM ".$ecs->table('user_bonus')." AS ub, ".$ecs->table('bonus_type')." AS bt ".
           "WHERE bt.type_id = ub.bonus_type_id AND ub.bonus_type_id = '$tid' ORDER BY ub.bonus_id DESC";
    $res = $db->query($sql);
    ...
    {
        echo $val['bonus_sn'] . "\t";
		echo decrypt($val['bonus_pwd']) ."\t";
		echo (empty($val['supplier_id']) ? '本网店' : $db->getOne('SELECT supplier_name FROM '.$ecs->table('supplier')." WHERE supplier_id = '".intval($val['supplier_id'])."'"))."\t";
        echo $val['type_money'] . "\t";
...
//导入线下红包
if ($_REQUEST['act'] == 'import_excel'){
	$smarty->assign('tid',    intval($_REQUEST['tid']));
...
if ($_REQUEST['act'] == 'upload'){
	/* 将文件按行读入数组，逐行进行解析 */
	$line_number = 0;
	$arr_data = file($_FILES['file']['tmp_name']);
	$arr_uploadFiled = array(
		'bonus_sn'  => '红包序列号',
		'bonus_pwd' => '红包密码',
	);
	$arr = array();
	$arr_field = array_keys($arr_uploadFiled);
	foreach ($arr_data AS $line){
		...
		for ($i = 0; $i < $len; $i++)
		{
			...
			elseif (',' == $char)
			{
				if (0 == $quote)
				{
					if (!isset($arr_field[count($arr)]))
					{
						continue;
					}
					$field_name = $arr_field[count($arr)];
	...
	// 字段名称列表
	$smarty->assign('title_list', $arr_uploadFiled);
	// 显示的字段列表
	$smarty->assign('field_show', array('bonus_sn' => true, 'bonus_pwd' => true));
	/* 参数赋值 */
	$smarty->assign('ur_here', '红包导入确认');
	$smarty->assign('tid',    intval($_REQUEST['tid']));
	/* 显示模板 */
	assign_query_info();
	$smarty->display('bonus_confirm.htm');
}

if ($_REQUEST['act'] == 'import_insert'){
	if (isset($_POST['checked'])){
		...
		foreach ($_POST['checked'] AS $key => $value){
			$arr_data = array(
				'bonus_type_id' => intval($_REQUEST['tid'])
			);
			foreach ($arr_field AS $field){
				...
				if (!empty($field_value)){
					if ($field == 'bonus_pwd'){
						$arr_data[$field] = encrypt($field_value);
					}else{
						$arr_data[$field] = $field_value;
					}
				}
			}
			$db->autoExecute($ecs->table('user_bonus'), $arr_data, 'INSERT');
			$int_index++;
		}
	}
	...
	/* 提示信息 */
	$link[0]['text'] = $_LANG['back_bonus_list'];
	$link[0]['href'] = 'bonus.php?act=bonus_list&bonus_type=' . intval($_REQUEST['tid']);

	sys_msg($_LANG['creat_bonus'] . $int_index . $_LANG['creat_bonus_num'], 0, $link);
}
...
function get_type_list()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['supplier_name'] = (empty($row['supplier_id']) ? '本网店' : $GLOBALS['db']->getOne('SELECT supplier_name FROM '.$GLOBALS['ecs']->table('supplier')." WHERE supplier_id = '".intval($row['supplier_id'])."'"));
...
function get_bonus_list()
{
    ...
    foreach ($row AS $key => $val)
    {
		...
		$row[$key]['bonus_pwd'] = decrypt($val['bonus_pwd']);
    }
...
function getBonusPwd($int_mmgz, $int_length = 8){
	$str_pwd = '';
	switch ($int_mmgz){
		case 2:
			$str_chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPRQSTUVWXYZ0123456789";
			while (strlen($str_pwd) < $int_length) {
				$str_pwd .= $str_chars[mt_rand(0, strlen($str_chars))];
			}
			break;
		case 1:
		default:
