                ...
                <div class="col-sm-12">
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            <a  class="btn btn-primary" style="margin-right: 20px;margin-top:10px;float: right" href="<?php echo U('Games/gameAdd');?>">新增游戏</a>
                            <table class="table table-striped" cellpadding=3 cellspacing=5>
                                ...
                                <?php if(is_array($game_list)): $i = 0; $__LIST__ = $game_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td>
                                            <a  href="<?php echo U('Games/gameEdit','id='.$vo['id']);?>">
                                                <button class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></button>
                                            </a>
                                            <a href="<?php echo U('Games/gameDelete','id='.$vo['id']);?>" onclick="return confirm('确定将此记录删除?')">
                                                <button class="btn btn-danger btn-xs"><i class="fa fa-trash-o "></i></button>
                                            </a>
                                        </td>
                                    </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                            </table>
                            <div class="dataTables_paginate paging_bootstrap"><?php echo ($pages); ?></div>
                        </div>
                    </section>
                </div></div>
            <!-- page end-->
...
$(document).on('click','#btn_status',function () {
    ...
    var url = '<?php echo U("Games/ajaxStatusChange");?>';
//    alert(id+'-'+status);
    $.ajax({
        type:"post",
        url:url,
