...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if ($is_add)
    {
        ...
        $goods = array(
            ...
            'goods_weight'  => 0,
            'give_integral' => -1,
            'limit_num' => $_REQUEST['limit_num']?$_REQUEST['limit_num']:'',
            'sms_content_id' => $_REQUEST['sms_content_id']?$_REQUEST['sms_content_id']:'',
            'rank_integral' => -1
        );
    ...
    else
    {
        ...
        $goods = $db->getRow($sql);
        if($goods['cat_id']=='91'){
            $sms_content=$db->getALl("SELECT * FROM " . $ecs->table('sms_content'));
            $goods['sms_content']=$sms_content;
        ...
        /* 供应商折扣 */
        // 商店折扣比例
        $shop_ratio = suppliers_ratio('shop_ratio',$goods['supplier_id']);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $limit_num = isset($_POST['limit_num']) ? $_POST['limit_num'] : '';
    $sms_content_id = isset($_POST['sms_content_id']) ? $_POST['sms_content_id'] : '';
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "seller_note, goods_weight, goods_number, goods_num, warn_number,limit_num,sms_content_id, integral, give_integral, is_best, is_new, is_hot, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$warn_number','$limit_num', '$sms_content_id','$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
        ...
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, goods_num, warn_number,limit_num,sms_content_id, integral, give_integral, is_best, is_new, is_hot, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number', '$goods_num',".
                    " '$warn_number','$limit_num','$sms_content_id', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
	...
	//删除商品货号
	$sql = "DELETE FROM " . $ecs->table('goods_huohao') . " WHERE goods_id = '$goods_id'";
	$db->query($sql);
