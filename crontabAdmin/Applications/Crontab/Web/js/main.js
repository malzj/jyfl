...
url = 'crontabAdmin/Applications/Crontab/Web/autoCron.php?fun=';
...
function read_log(){
    $.ajax({
        type: 'GET',
        cache: false,
        url: 'http://'+document.domain+port+'/'+url+'read_log',
...
function clear_log(){
    $.ajax({
        type: 'GET',
        cache: false,
        url: 'http://'+document.domain+port+'/'+url+'clear_log',
        success: function(data){
            read_log();
        }
    })
...
function get_status(){
    $.ajax({
        type: 'GET',
        cache: false,
        url: 'http://'+document.domain+port+'/'+url+'get_status',
        success: function(data){
            var obj = eval("("+data+")");
...
function add_blacklist(obj, file){
    $.ajax({
        type: 'GET',
        cache: false,
        url: 'http://'+document.domain+port+'/'+url+'add_blacklist/'+base64encode(file),
        success: function(data){
            if(data == 'success')
            {
                read_log();
...
function remove_blacklist(obj, file){
    $.ajax({
        type: 'GET',
        cache: false,
        url: 'http://'+document.domain+port+'/'+url+'remove_blacklist/'+base64encode(file),
        success: function(data){
            if(data == 'success')
            {
                read_log();
...
function remove_task(obj, file){
    if(confirm('确定要删除'+file+'吗？如果不想删除请使用黑名单功能,点击确定直接删除')){
        $.ajax({
            type: 'GET',
            cache: false,
            url: 'http://'+document.domain+port+'/'+url+'remove_task/'+base64encode(file),
            success: function(data){
                if(data == 'success')
                {
                    read_log();
...
function start_task(obj, file){
    ...
    $.ajax({
        type: 'GET',
        cache: false,
        url: 'http://'+document.domain+port+'/'+url+'start_task/'+base64encode(file),
        success: function(data){
            if(data == 'success'){
...
function stop_task(obj, file){
    ...
    $.ajax({
        type: 'GET',
        cache: false,
        url: 'http://'+document.domain+port+'/'+url+'stop_task/'+base64encode(file),
        success: function(data){
            if(data == 'success'){
                read_log();
                get_status();
...
function start_all(){
    $.ajax({
        type: 'GET',
        cache: false,
        url: 'http://'+document.domain+port+'/'+url+'start_all',
        success: function(data){
            if(data == 'success'){
                setTimeout(function(){read_log();}, 1000);
                get_status();
...
function stop_all(){
    if(confirm('确定要停止所有任务吗？部分任务需要使用黑名单功能停止')){
        $.ajax({
            type: 'GET',
            cache: false,
            url: 'http://'+document.domain+port+'/'+url+'stop_all',
            success: function(data){
                if(data == 'success'){
                    read_log();
                    get_status();
