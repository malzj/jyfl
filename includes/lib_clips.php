...
function insert_user_account($surplus, $amount)
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
           ' (user_id, order_sn, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid)'.
            " VALUES ('$surplus[user_id]', '$surplus[order_sn]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0)";
    $GLOBALS['db']->query($sql);
...
function get_online_payment_list($include_balance = true)
{
    $sql = 'SELECT pay_id, pay_code, pay_name, pay_fee, pay_desc ' .
            ...
            " WHERE enabled = 1 AND is_cod <> 1 AND is_wap <> 1";
    if (!$include_balance)
    {
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points,youxiao_time, user_money, credit_line, last_login, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    ...
    $info['youxiao_time'] = $row['youxiao_time'];
    if (strpos($info['youxiao_time'], '-') === false)
    {
    	$year = substr($info['youxiao_time'], 0, 4);
    	$month = substr($info['youxiao_time'], 4, 2);
    	$day = substr($info['youxiao_time'], 6, 2);
    	$info['youxiao_time'] = $year.'-'.$month.'-'.$day;
    }
...
//最近一个月的订单列表
function get_order_months($user_id, $num = 10, $start = 0){
    /* 取得订单列表 */
    $arr    = array();

    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, consignee, province, city, district, address, tel, mobile, add_time, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
           " FROM " .$GLOBALS['ecs']->table('order_info') .
           " WHERE user_id = '$user_id' AND add_time > '".local_strtotime('-1 months')."' ORDER BY add_time DESC";
    $res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$sql = "SELECT og.goods_id,og.goods_name,g.goods_thumb,og.goods_sn FROM " .$GLOBALS['ecs']->table('order_goods') ." og ".
			"left join " .$GLOBALS['ecs']->table('goods') ." g on g.goods_id = og.goods_id " .
           " WHERE og.order_id = ".$row['order_id'];
		$row['order_goods_list'] = $GLOBALS['db']->getAll($sql);
		foreach ($row['order_goods_list'] as $key=>$goods){
			$goods['goods_style_fu_name'] = add_style($goods['goods_fu_name'], $goods['goods_fu_name_style']);
			$goods['url'] = build_uri('goods', array('gid'=>$goods['goods_id']));
			$row['order_goods_list'][$key] = $goods;
		}
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'province'       => get_add_cn($row['province']),
                       'city'           => get_add_cn($row['city']),
                       'district'       => get_add_cn($row['district']),
                       'address'        => $row['address'],
                       'mobile'         => $row['mobile'],
                       'tel'            => $row['tel'],
                       'total_fee'      => price_format($row['total_fee'], false),
                       'handler'        => $row['handler'],
					   'order_goods_list' => $row['order_goods_list']);
    }
