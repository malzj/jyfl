...
/* 在线选座下单 */
	...
	if ($diffPrice == -1){
	    $jsonArray['state'] = 'false';
	    $jsonArray['message'] = '您的剩余次数不足，请修改！';
	    JsonpEncode($jsonArray);
	}
...
/* 支付电影票 */
    ...
    $isTimes = intval($_REQUEST['isTimes']);
	// 如果是微信支付后跳转过来的话，获取订单id和卡密码
	if ($isTimes == 1){
	    $int_orderId = $_SESSION['weixinpay']['order_id'];
	    $str_password = $GLOBALS['db']->getOne("SELECT card_password FROM " . $GLOBALS['ecs']->table('users') ." WHERE user_id = '$_SESSION[user_id]'");;
	}
	else{
	    $int_orderId = intval($_REQUEST['order_id']);
	...
	if ($state == 0){
		...
		if( isset($_SESSION['weixinpay']) )
		      unset($_SESSION['weixinpay']);
...
else if($_REQUEST['act'] == 'weixinpay')
{
    $_SESSION['weixinpay']['order_id'] = $_REQUEST['order_id'];
    JsonpEncode($jsonArray);
}