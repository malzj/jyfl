...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    if ($filename != 'index')
    {
        ...
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            {
                krsort($cat_arr);
                $shift = array_shift($cat_arr);
                array_unshift($cat_arr, $shift);
                foreach ($cat_arr AS $val)
                {
                    $page_title = htmlspecialchars($val['cat_name']) . '_' . $page_title;
                    $args       = array($key => $val['cat_id']);
                   	if($shift['cat_id'] == $val['cat_id'])
                    {
                    	$ur_here   .= ' <code>&gt;</code>' .htmlspecialchars($val['cat_name']);
                    }
                    else
                    { 
...
function get_shop_help()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'ON a.cat_id = c.cat_id WHERE c.cat_type = 4 AND a.is_open = 1 ' .
...
function assign_template($ctype = '', $catlist = array())
{
	...
	$arr_homeInfo = $arr_nav['home'];
	unset($arr_nav['home']);
	$arr_nav['middle'] = array_reverse($arr_nav['middle']);
    $smarty->assign('navigator_list',       $arr_nav);  //自定义导航栏
	$smarty->assign('homeinfo',       $arr_homeInfo);  //自定义导航栏

	if ($GLOBALS['str_scriptName'] != 'home' && $arr_homeInfo['home_desc'] && empty($_SESSION['is_home'])){
	...
	//$smarty->assign('yundong',    get_navigator_top(3)); //运动激情
	//$smarty->assign('zhihui',     get_navigator_top(4)); //智慧之门
	//$smarty->assign('yingji',     get_navigator_top(5)); //应季甄选
...
function article_categories_tree($cat_id = 0)
{
    ...
    */
        ...
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                ...
                "WHERE a.parent_id = '$cat_id' AND a.cat_type=1 ORDER BY parent_order ASC, a.cat_id ASC, child_order ASC";
...
function get_navigator($ctype = '', $catlist = array())
{
    ...
    foreach($navlist['middle'] as $k=>$v)
    {
		...
		$navlist['middle'][$k]['realCCount'] = getNavChild($v['id']);//实际有多少个子导航
    }
	...
	foreach($navlist['middle'] as $k=>$v){
		...
		if ($user_card_status == true && empty($arr_showNav))
		{
			break;
		}
		
		if (empty($arr_showNav)){
			$arr_cityRule = !empty($v['is_city']) ? unserialize($v['is_city']) : array();
			if (!in_array($int_cityId, $arr_cityRule)){
				continue;
			}
		}else{
			if (!in_array($v['id'], $arr_showNav)){
				continue;
			}
		}

		if (empty($v['menulist'])){
			$arr_middleNav[$v['id']] = $v;
		}else{
			$arr_middleNav[$v['menulist']]['currCCount'] += 1;//目前有多少个子导航
			$arr_middleNav[$v['menulist']]['child'][$v['id']] = $v;
		}
	}
	
	
	$navlist['middle'] = $arr_middleNav;
	!empty($_SESSION['user_id']) && $navlist['home'] = $arr_homeInfo;
    return $navlist;
}

function getNavChild($pid){
	if (empty($pid)) return 0;
	$sql = 'SELECT COUNT(*) FROM '. $GLOBALS['ecs']->table('nav') . '
			WHERE ifshow = \'1\' AND menulist = '.intval($pid);
	return (int)$GLOBALS['db']->getOne($sql);
}


function get_navigator_top($catid)
{

	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('category') .
            " WHERE parent_id = '$catid' " .
            'AND is_show = 1 ORDER BY sort_order';
    $res = $GLOBALS['db']->query($sql);
...
//根据城市id得到票工厂piaoduoduo_id
function getPiaoduoduoId($city=""){
    if($city){
        $sql="SELECT * FROM ".$GLOBALS['ecs']->table('region')." where region_id ='".$city."'";
        $row=$GLOBALS['db']->getRow($sql); 
    }else{
        $sql="SELECT * FROM ".$GLOBALS['ecs']->table('region')." where region_id =1";
        $row=$GLOBALS['db']->getRow($sql);  
    }
    return $row['piaoduoduo_id'];
...
//获取票工厂景区信息列表
// $reconds总条数 
// $code 地区piaoduoduo_id
function piaoduoduo_jingqu($code='', $page=1){

    $getScrnicInfoList = array();
    $lastPage = 0;
    $state = true;
    for($i=$page;$i<=($page+5);$i++){   

        $arr_param_jingqu=array(
            'pageindex'=>$i,
            'pagesize'=>20,   
            'regioncode'=>$code//所在地区Code
        );
        $piao=piaoduoduo('Agent_GetScenicInfoList',$arr_param_jingqu); 

        if( ceil($piao['head']['reconds'] / 20) < $i){
            $state = false;
            break;
        }
        if(empty($piao['API_Scenic'])){
            continue;
        }
        //单个景区的时候
        if(array_key_exists('SightName', $piao['API_Scenic'])){
            
            $getScrnicInfoList[$piao['API_Scenic']['SupplierKeyId']] = $piao['API_Scenic'];
            $getScrnicInfoList[$piao['API_Scenic']['SupplierKeyId']]['chanpin'] = getchanpin($piao['API_Scenic']['SupplierKeyId'],$code);
        //多个景区                   
        }else{
            // 多个景区
            foreach ($piao['API_Scenic'] as $key_s => $value_s){                 
                $getScrnicInfoList[$value_s['SupplierKeyId']] = $value_s;
                // 产品
                $getScrnicInfoList[$value_s['SupplierKeyId']]['chanpin'] = getchanpin($value_s['SupplierKeyId'],$code);
            }
        }   
    ...
    foreach ($getScrnicInfoList as $ke => &$infoLists) {
        // 去掉没有产品的景区
        if (is_null($infoLists['chanpin'])) {
            unset($getScrnicInfoList[$ke]);
            continue;
        }
        // 景区图片和最低价格
        $SellPrice = array();
        $scrnicImg = null;
        $scrnicImgs='';
        foreach ($infoLists['chanpin'] as $chanpinKey => $chanpinVal) {
            // 图片
            $imgs = $chanpinVal['Imgs'];
            if (strpos($imgs, '.') !== false && $scrnicImg == null) {
                if (strpos($imgs, ',') !== false) {
                    $scrnicImgs = explode(',',$imgs);
                    $scrnicImg = $scrnicImgs[0];
                }else{
                    $scrnicImg = $imgs;
                }
            }

            if (!getimagesize($scrnicImg)) {
                $scrnicImg = '/images/piaoduoduo/zanwu.jpg';
            }
            // 价格
            $SellPrice[] = price_format($chanpinVal['SellPrice']);
        }

        $SellPrice = array_filter($SellPrice);
        $infoLists['imgs'] = $scrnicImg;
        $infoLists['price'] = min($SellPrice);
        $infoLists['rows'] = count($SellPrice);
    }

    // 最后执行的页数
    if ($lastPage > 0) {
        include_once(ROOT_PATH . 'includes/lib_cardApi.php');
        $_SESSION['_nv'][$code] = $lastPage;
        $filename = ROOT_PATH. 'temp/piaoduoduo/piaoduoduo-'.$code.'.php';
        if (is_file($filename)) {
            $old = include_once(ROOT_PATH. 'temp/piaoduoduo/piaoduoduo-'.$code.'.php');
            $newInfoList = array_merge($getScrnicInfoList, $old);           
        }else{
            $newInfoList = $getScrnicInfoList;
        }


        mkcache("piaoduoduo/piaoduoduo-".$code,$newInfoList,"w");
        //F("piaoduoduo-".$code, $newInfoList, 1209600, "piaoduoduo/")
    }
    $code='';
    return $state;
  }       
...
//以产品KeyId获取产品信息，例如
//$KeyId='26e253f2-6118-43d7-9b60-8454621e7760';
//$code=110000;
function getchanpinOrder($KeyId,$code){
    $replace=array('星期一','星期二','星期三','星期四','星期五','星期六','星期日',',');
    $find=array('1','2','3','4','5','6','7','，');     
    if($KeyId){
         $arr_param_lists=array(
            'pageindex'=>1,
            'pagesize'=>20,
            'productId'=>$KeyId, //产品ID 
            // 'SupplierKeyId'=>$SupplierKeyId,//景区ID
            // 'productName'=>"中景信",//产品名称 
            'regionCode'=>$code//所在地区Code
        );
        $piao_chanpin = piaoduoduo('Agent_GetProductList',$arr_param_lists);//获取产品信息列表
    
        $return = array();
       
        // 一个产品的时候
        if(array_key_exists('ProductName', $piao_chanpin['API_Product'])){
            $return[0] = $piao_chanpin['API_Product'];
            $return[0]['SettlementPrice']=price_format($piao_chanpin['API_Product']['SettlementPrice']);
            $return[0]['MarketPrice']=price_format($piao_chanpin['API_Product']['MarketPrice']);
            $return[0]['PushPrice']=price_format($piao_chanpin['API_Product']['PushPrice']);
            $return[0]['SellPrice']=price_format($piao_chanpin['API_Product']['SellPrice']);
            $return[0]['Imgs']=explode(",", $piao_chanpin['API_Product']['Imgs']);
            $return[0]['PeriodValidityOfRefund']=$piao_chanpin['API_Product']['PeriodValidityOfRefund']?'自出行之日起'.$piao_chanpin['API_Product']['PeriodValidityOfRefund'].'日内可退票':'不可退票';
            $return[0]['UseAdvanceHour']=$piao_chanpin['API_Product']['UseAdvanceHour']?'预定后'.$piao_chanpin['API_Product']['UseAdvanceHour'].'小时才能入园':'即刻入园';
            $return[0]['ValidWeek']=$piao_chanpin['API_Product']['ValidWeek']? '仅限'.str_replace($find,$replace,$piao_chanpin['API_Product']['ValidWeek']).' 使用':'';

            if($piao_chanpin['API_Product']['TimeZone']==1){
                 $return[0]['TimeZone']='平日使用';   
            }elseif($piao_chanpin['API_Product']['TimeZone']==2){
                $return[0]['TimeZone']='周末使用';
            }elseif($piao_chanpin['API_Product']['TimeZone']==4){
                $return[0]['TimeZone']='不限制使用时间';
            }else{
                $return[0]['TimeZone']='';
            }
        ...
        // 多个产品的时候
            $return = $piao_chanpin['API_Product'];
            foreach ($return as $key_arr => $value_arr) {
               
                if($value_arr){

                    $return[$key_arr]['SettlementPrice']=price_format($value_arr['SettlementPrice']);
                    $return[$key_arr]['MarketPrice']=price_format($value_arr['MarketPrice']);
                    $return[$key_arr]['PushPrice']=price_format($value_arr['PushPrice']);
                    $return[$key_arr]['SellPrice']=price_format($value_arr['SellPrice']);
                    $return[$key_arr]['Imgs']=explode(",", $value_arr['Imgs']);
                    $return[$key_arr]['PeriodValidityOfRefund']=$value_arr['PeriodValidityOfRefund']?'自出行之日起'.$value_arr['PeriodValidityOfRefund'].'日内可退票':'不可退票';
                    $return[$key_arr]['UseAdvanceHour']=$value_arr['UseAdvanceHour']?'预定后'.$value_arr['UseAdvanceHour'].'小时才能入园':'即刻入园';
                    $return[$key_arr]['ValidWeek']=$value_arr['ValidWeek']? '仅限'.str_replace($find,$replace,$value_arr['ValidWeek']).' 使用':'';
                    if($value_arr['TimeZone']==1){
                         $return[$key_arr]['TimeZone']='平日使用';   
                    }elseif($value_arr['TimeZone']==2){
                        $return[$key_arr]['TimeZone']='周末使用';
                    }elseif($value_arr['TimeZone']==4){
                        $return[$key_arr]['TimeZone']='不限制使用时间';
                    }else{
                        $return[$key_arr]['TimeZone']='';
                    }
...
//以景区SupplierKeyId获取产品信息，例如
//$SupplierKeyId='91decc83-4702-ce93-27eb-08d29e30ed22';
//$code=110000;
function getchanpin($SupplierKeyId,$code){
    if($SupplierKeyId){
        $arr_param_lists=array(
            'pageindex'=>1,
            'pagesize'=>20,
        ...
        $piao_chanpin = piaoduoduo('Agent_GetProductList',$arr_param_lists);//获取产品信息列表
        // 产品为为空，价格和图片为0;
        if (empty($piao_chanpin['API_Product'])) {
            return null;
        }
        ...
        // 多个产品的时候
            // $return = $piao_chanpin['API_Product'];
            foreach ($piao_chanpin['API_Product'] as $key => $value) {
...
//$KeyId='26e253f2-6118-43d7-9b60-8454621e7760';
//$code=110000;
function getSupplierKeyId($SupplierKeyId='',$KeyId='',$code){
    $ratio = get_card_rule_ratio(10004);
    if($SupplierKeyId||$KeyId){
         $arr_param_lists=array(
            'pageindex'=>1,
            'pagesize'=>20,
            'SupplierKeyId'=>$SupplierKeyId,//景区ID
        ...
        if(array_key_exists('ProductName', $piao_chanpin['API_Product'])){
            ...
            //  if ($ratio !== false)
            // {           
            //     $return[0]['SellPrice'] = price_format($piao_chanpin['API_Product']['SellPrice']*$ratio);  // TODO 价格
            // }
            // // 不存在卡规则比例，设置默认价格
            // else
            // {           
            //     $return[0]['SellPrice'] = interface_price($piao_chanpin['API_Product']['SellPrice'],'piaoduoduo');           
            // } 
            $return[0]['SellPrice'] = $piao_chanpin['API_Product']['SellPrice'];           
            $return[0]['SettlementPrice']=price_format($piao_chanpin['API_Product']['SettlementPrice']);
            $return[0]['MarketPrice']=price_format($piao_chanpin['API_Product']['MarketPrice']);
            $return[0]['PushPrice']=price_format($piao_chanpin['API_Product']['PushPrice']);
            // $return[0]['SellPrice']=price_format($piao_chanpin['API_Product']['SellPrice']);
        ...
        // 多个产品的时候
            ...
            foreach ($return as $key_arr => $value_arr) {
                ...
                if($value_arr){
                    // if ($ratio !== false)
                    // {           
                    //     $return[$key_arr]['SellPrice'] = price_format($value_arr['SellPrice']*$ratio);  // TODO 价格
                    // }
                    // // 不存在卡规则比例，设置默认价格
                    // else
                    // {           
                    //     $return[$key_arr]['SellPrice'] = interface_price($value_arr['SellPrice'],'piaoduoduo');           
                    // }
                    $return[$key_arr]['SellPrice'] = $value_arr['SellPrice'];  // TODO 价格
                    $return[$key_arr]['SettlementPrice']=price_format($value_arr['SettlementPrice']);
                    $return[$key_arr]['MarketPrice']=price_format($value_arr['MarketPrice']);
                    $return[$key_arr]['PushPrice']=price_format($value_arr['PushPrice']);
                    // $return[$key_arr]['SellPrice']=price_format($value_arr['SellPrice']);
...
// $filename  存储目录
// $write  w为写，r为读
function mkcache($filename,$arr='',$wirte="w"){

    if($wirte=="w"){
        if($arr){
            if($filename){
                $filenames="./temp/".$filename.".php";
                $dir            =   dirname($filenames);
                // 目录不存在则创建
                if (!is_dir($dir))
                    mkdir($dir,0755,true);
                return file_put_contents($filenames, "<?php\treturn " . var_export($arr, true) . ";?>");

            }else{
                $filenames="./temp/".mt_rand(1,9999).".php";
                $myfile = fopen($filenames, "w+") or die("创建失败");
                $txt="<?php\treturn " . var_export($arr, true) . ";\n";
                fwrite($myfile, $txt);
                fclose($myfile);
            }
         }
    }else{
         $res = include_once('/temp/'.$filename.".php");
         return $res;
    }
}

// 景区列表
function piaoduoduo_jingqu_list($code)
{
    $code=$code?$code:'1';
    /* 查询 */
    $sql = "SELECT j.*,c.class_name "."FROM " . $GLOBALS['ecs']->table('piaoduoduo_jingqu') . " AS j"." LEFT JOIN " . $GLOBALS ['ecs']->table ( 'piaoduoduo_class' ) . " AS c ON c.class_id=j.class_id "."where is_class=1 and region_id='".$code."'" ;
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $city[]=end(explode('-', $row['City']));//地区
        $fenlei[]=$row['class_name'];//地区
    ...
    $arr = array('list' => $list, 'city' => array_merge(array('全部'),array_unique($city)),'fenlei' => array_merge(array('全部'),array_filter(array_unique($fenlei))));
    return $arr;
}
