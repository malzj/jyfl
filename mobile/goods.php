...
if(in_array($catArr['cat_id'], array(91,92))){
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    else
    {
        ...
        // ====================================================== 
        $res['result'] = price_format($new_price * $number);
    }

	$res['attr_id'] = $_REQUEST['attr'];
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'pic'){
	include('includes/cls_json.php');

	$json   = new JSON;
	$res    = array('err_msg' => '', 'result' => array(), 'qty' => 1);
	$id    = $_REQUEST['id'];
	$img_id = $_REQUEST['img_id'];
	$img = get_img_url($img_id,$id);
	$res['result1'] = $img['img_url'];
	$res['result2'] = $img['img_original'];
	die($json->encode($res));
}
...
if (!$smarty->is_cached('goods.html', $cache_id))
{
    ...
    else
    {
		...
		$cat = get_cat_info($goods['cat_id']);   // 获得分类的相关信息
        ...
        $smarty->assign('specs',                $spec_array);
...
$smarty->assign('header', get_header('商品详情', true, true));
