...
function get_admin_logs()
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
		...
		$rows['user_name'] = empty($rows['is_supplier']) ? $rows['user_name'] : $GLOBALS['db']->getOne('SELECT supplier_name FROM ' .$GLOBALS['ecs']->table('supplier'). " WHERE supplier_id = '".intval($rows['user_id'])."'");
        $list[] = $rows;
    }
