var data_url ="http://196.168.1.161/jyflapi/";
$(function(){
...
function saveAddress(address_id){
	...
	$.ajax({
		type:"post",
		url:data_url+"index.php/Users/User/saveAddress",
		data:"user_id="+user_id+'&address_id='+address_id,
		dataType:'json',
		success:function(data){
			$("option[value="+data.business.city+"]").attr("selected", true);
			$("#hstreet").val(data.business.address);
			$("#zipcode").val(data.business.zipcode);
			$("#consignee").val(data.business.consignee);
			$("#tel").val(data.business.mobile);
			$("#address_id").val(data.business.address_id);
		}
	});
}
...
function showprovince(){
	var option = "<option style='width:100px;background-color:black;' display='inline' value='0' >请选择城市</option>";
