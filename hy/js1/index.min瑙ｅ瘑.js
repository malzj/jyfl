...
function(a) {
    ...
    var e, f, g = ["wheel", "mousewheel", "DOMMouseScroll", "MozMousePixelScroll"],
        h = "onwheel" in document || document.documentMode >= 9 ? ["wheel"] : ["mousewheel", "DomMouseScroll", "MozMousePixelScroll"], //h 存放鼠标事件类型，documentMode 是一个IE的私有属性，在IE8+中被支持。
        i = Array.prototype.slice; //存放slice
    if (a.event.fixHooks)
...
(function(e) {
    ...
    var o = "left",
        ...
        g = "end",
        p = "cancel",
        a = "ontouchstart" in window, //判断是pc还是手机
        y = "TouchSwipe";
    var m = {
        fingers: 1,
        threshold: 75,
...
/***以下是解密还原之后的代码  使用工具：http://tool.lu/js/***/ 
! function() {
    var pageIndex = 0,
        preloadCount = 0;
    ...
    function pageLoad() {
        ...
        $("header .menu li a").bind("mouseenter", function() {
            ...
            line.stop().animate({ width: $(this).width() + 10, left: parseInt($(this).position().left) - 5 + "px" }, 300)
        });
        ...
        videoSwiper = new Swiper(".video .swiper-container", {
            loop: true,
            autoplay:false, //控制自动播放的时间（单位ms），false不自动播放
            grabCursor: true,
            onSlideChangeStart: function() { resetVideoSwiperAnimation() },
            onSlideChangeEnd: function() {
                $(".video .guide a").removeClass("active").eq(videoSwiper.activeLoopIndex).addClass("active");
                videoSwiperAnimation()
            },
            onTouchEnd: function() { videoSwiper.startAutoplay() }
        });
        ...
        qualitySwiper = new Swiper(".quality .swiper-container.mini", { loop: true, autoplay: 5000, grabCursor: true, onTouchEnd: function() { qualitySwiper.startAutoplay() } });
        ...
        aboutSwiper = new Swiper(".aboutus .swiper-container.items", {
            ...
            onTouchEnd: function() { aboutSwiper.startAutoplay() }
        });
        ...
        }).bind("mouseout", function() { aboutSwiper.startAutoplay() }).eq(0).addClass("active");
        ...
        if (tEvent == undefined) {
            ...
            $("div.welcome").animate({ opacity: "0", zIndex: "-1" }, 500, "swing", function() {
                initLayout();
                pageSwitching()
            })
        }
    ...
    function pageSwitching() {
        ...
        var lock = false;
        $("html,body").stop().animate({ scrollTop: $("body>section").eq(pageIndex).offset().top }, 300, "swing", function() {
            if (lock) return;
            else lock = true;
