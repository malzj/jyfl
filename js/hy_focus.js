...
indexSlides.obj=$('#slide-index .slide_dt');
indexSlides.style=[];
indexSlides.style[0]={
...
indexSlides.goto=function(i){
	...
	$('#slide-index .slides').animate({'margin-left':left},500,function(){ //2张图片直接的间隔
		$obj.find('.text').animate(indexSlides.css[j].text,300,function(){ //图片的停留时间
			$obj.find('.button').show().animate(indexSlides.css[j].button,300, "swing",function(){
				
				if(i>=indexSlides.css.length){
					$obj=$('#slide-index .slide_dt').eq(0);
...
indexSlides.start=function(){
	indexSlides.timer=setInterval(indexSlides.next,3500);//切换的时间
}
...
indexSlides.reformat=function(){
	...
	if(!indexSlides.animating){
		...
		$obj=$('#slide-index .slide_dt').eq(indexSlides.current);
