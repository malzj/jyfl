...
function addToCart(goodsId, parentId, carttype){
	...
	$(this).loadAjax('flow.php', {step:"add_to_cart", goods:JSON.stringify(goods), t:Math.random()}, function(data){
		data = jQuery.parseJSON(data)
		addToCartResponse(data);
	});
	/*jQuery.post("flow.php",{step:"add_to_cart", goods:JSON.stringify(goods), t:Math.random()}, function(data){
		//data = jQuery.parseJSON(data);
		addToCartResponse(data);
	}, 'json');*/
}
...
function addToCartResponse(result){	
	...
	if (result.error > 0){
		...
		//没选规格，弹出属性选择框
			...
			openSpeDiv(result.message, result.goods_id, result.parent, result.carttype);
		}
		else{
		...
		if (result.one_step_buy == '1'){
			...
			case '1' :
				layer.msg('成功加入购物车',{skin:"layui-layer-huimsg",shift:1, offset:'60px'});
				break;
			case '2' :alert(2);
